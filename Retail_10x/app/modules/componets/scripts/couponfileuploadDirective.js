...
angular.module('Retails.couponfileuploadDirective', ['shared.serviceFactory'])

  .directive('couponfileuploadDirective', ['$compile', '$window','serviceFactory', '$timeout','$rootScope','$filter',
...
function deleteFile(albumName,fileName) {
 ...
 var fileuploadevent;
      ...
      fileuploadevent= $rootScope.$on('addcouponfile', function (event, data) {
        
        if(data.title=="addfile"){
         addPhoto(scope.folderName+"coupons/"+data.campaignId,data.file,data.quidfileName);
        }
        else if(data.title=="deletefile"){
          deleteFile("coupons",data.file,data.quidfileName)
        }
