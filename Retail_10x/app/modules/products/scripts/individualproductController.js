
'use strict';

/**
 * @ngdoc controller
 * @name ProductModule.productController
 * @requires $scope
 * @requires $state
 * @requires product.serviceFactory
 * @description
 *
 *
 * */

angular.module('individualproduct.controllers',
  ['Retails.barchartDirective','Retails.salesperformance'])


  .controller('individualproductCtrl', ['$scope', '$state', 'serviceFactory','productService','dashBoardService','$http','$timeout','usSpinnerService', '$rootScope','$filter',
    function ($scope, $state, serviceFactory,productService,dashBoardService,$http,$timeout,usSpinnerService, $rootScope,$filter) {

    if (sessionStorage.user == undefined || sessionStorage.user == null||sessionStorage.user =="null") {
      $state.go('login');
    }

      $scope.mfgId=sessionStorage.mfgId;
        $scope.departmentDetail=productService.getdeptDetail()
      $scope.masked = false;

      $scope.individualcategoryproduct=false;
      $scope.itemsList=[];
      // $scope.getcompproduct=[];
      $scope.getcompproduct=productService.getselectedproduct();
      
      //console.log("selected product from detail page",$scope.getcompproduct);
      
      // .........search product.............

    $scope.nutella = "data:image/jpeg;base64,";
    $scope.raffaello = "data:image/jpeg;base64,";
    $scope.ticTac = "data:image/jpeg;base64,";
    $scope.defaultProduct = "data:image/jpeg;base64,";
    $scope.tide = "data:image/png;base64,";
    
      $scope.checkduplicateTypehead = function(typeaheadobject){
        var flag =  true;
       $scope.typeaheadList.forEach(function (item) {
          if(item.upc === typeaheadobject.upc){
            flag = false;
          }
      });
       return flag;
    }

    $scope.GetProductList = function (value) {
       sessionStorage.productsuggested = value;
       if($scope.role=="retailer"){
         return productService.GetProductSuggested().then(function (response) {
            $scope.listOfProducts = response.data;
           // console.log(" responsedata.............", response.data);
            $scope.typeaheadList = [];
            for (var i = 0; i < response.data.length; i++) {
              var tictac = response.data[i].product.toLowerCase().indexOf('tictac');
              var nutella= response.data[i].product.toLowerCase().indexOf('nutella');
              var ferrero= response.data[i].product.toLowerCase().indexOf('ferrero');
              var tide   = response.data[i].product.toLowerCase().indexOf('tide');

              var typeaheadobject = {
                "image": $scope.defaultProduct,
                "product": response.data[i].product,
                "upc": response.data[i].upc,
                "price": response.data[i].size,
                "deptId":response.data[i].deptId,
                "category":response.data[i].categoryName,
                "deptName":response.data[i].deptName
              }

              if (nutella >= 0) {
                typeaheadobject = {
                  "image": $scope.nutella,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }

              }

              else if (ferrero >= 0) {
                typeaheadobject = {
                  "image": $scope.raffaello,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }

              else if (tictac >= 0) {
                typeaheadobject = {

                  "image": $scope.ticTac,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (tide >= 0) {
                typeaheadobject = {

                  "image": $scope.tide,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              $scope.typeaheadList.push(typeaheadobject);
            }
            return $scope.typeaheadList;
          }, function (response) {
            console.log(response);
           }
          );
        }
        else if($scope.role=="distributor"){           
           return productService.getproductSuggested().then(function (response) {
            $scope.listOfProducts = response.data;
            $scope.typeaheadList = [];
            for (var i = 0; i < response.data.length; i++) {
              var tictac = response.data[i].product.toLowerCase().indexOf('tictac');
              var nutella = response.data[i].product.toLowerCase().indexOf('nutella');
              var ferrero = response.data[i].product.toLowerCase().indexOf('ferrero');
              var tide = response.data[i].product.toLowerCase().indexOf('tide');
              var typeaheadobject = {
                "image": $scope.defaultProduct,
                "product": response.data[i].product,
                "upc": response.data[i].upc,
                "price": response.data[i].size,
                "deptId":response.data[i].deptId,
                "category":response.data[i].categoryName,
                "deptName":response.data[i].deptName
              }
              if (nutella >= 0) {
                typeaheadobject = {
                  "image": $scope.nutella,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (ferrero >= 0) {
                typeaheadobject = {
                  "image": $scope.raffaello,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (tictac >= 0) {
                typeaheadobject = {
                  "image": $scope.ticTac,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (tide >= 0) {
                typeaheadobject = {
                  "image": $scope.tide,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
               if($scope.checkduplicateTypehead(typeaheadobject)){
               $scope.typeaheadList.push(typeaheadobject);
              }
            }
            return $scope.typeaheadList;
          }, function (response) {
            console.log(response);
          }
        );
        }
        else {
        return productService.ProductsSuggestedforallretailer().then(function (response) {
            $scope.listOfProducts = response.data;
            $scope.typeaheadList = [];
            for(var i = 0; i < response.data.length; i++) {
              var tictac = response.data[i].product.toLowerCase().indexOf('tictac');
              var nutella = response.data[i].product.toLowerCase().indexOf('nutella');
              var ferrero = response.data[i].product.toLowerCase().indexOf('ferrero');
              var tide = response.data[i].product.toLowerCase().indexOf('tide');
              var typeaheadobject = {
                "image": $scope.defaultProduct,
                "product": response.data[i].product,
                "upc": response.data[i].upc,
                "price": response.data[i].size,
                "deptId":response.data[i].deptId,
                "category":response.data[i].categoryName,
                "deptName":response.data[i].deptName
              }
              if (nutella >= 0) {
                typeaheadobject = {
                  "image": $scope.nutella,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (ferrero >= 0) {
                typeaheadobject = {
                  "image": $scope.raffaello,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (tictac >= 0) {
                typeaheadobject = {
                  "image": $scope.ticTac,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
              else if (tide >= 0) {
                typeaheadobject = {
                  "image": $scope.tide,
                  "product": response.data[i].product,
                  "upc": response.data[i].upc,
                  "price": response.data[i].size,
                  "deptId":response.data[i].deptId,
                  "category":response.data[i].categoryName,
                  "deptName":response.data[i].deptName
                }
              }
               if($scope.checkduplicateTypehead(typeaheadobject)){
               $scope.typeaheadList.push(typeaheadobject);
              }
            }
            return $scope.typeaheadList;
          }, function (response) {
            console.log(response);
          }
        );
        }
      }
    $scope.productselected=function(value){
        $scope.prdctdetail=[];
      for(var i=0;i<$scope.typeaheadList.length;i++){
        if($scope.typeaheadList[i].upc==value){
            $scope.selectedproductfromtypeahead=$scope.typeaheadList[i];
            $scope.product=$scope.typeaheadList[i].product;
          }
        }
        if($scope.role=="retailer"){
          //console.log("............selectedproductfromtypeahead............",$scope.selectedproductfromtypeahead);
      window.localStorage['iddept']=  $scope.selectedproductfromtypeahead.deptId;
      productService.setCategoryforRetailer($scope.selectedproductfromtypeahead.category);
     // for(var j=0;j<$scope.departmentList.length;j++){
      //if($scope.departmentList[j].departmentNumber==parseInt($scope.selectedproductfromtypeahead.deptId)){
      //$scope.departmentdescription=$scope.departmentList[j].departmentDescription;
              window.localStorage['dataplus']= "plus";
              window.localStorage['checkflag']="";
              $scope.screen4 = $scope.selectedproductfromtypeahead.product;
              $scope.iddept=$scope.selectedproductfromtypeahead.deptId;
              $scope.itemNum=$scope.selectedproductfromtypeahead.upc;
              $scope.Namedept=$scope.departmentdescription;
              var data ={
              "deptId":$scope.iddept,
              "itemName":$scope.itemNum,
              "productName":$scope.screen4,
              "status":"plus"            
             }
             var results=$filter('filter')($scope.productList,{itemName:data.itemName}, true);
             if(results.length>0){
              alert("Product already Added");
             }
            else{
                $scope.productList.push(data);
                $scope.selectedProduct(data);
            }
           // }
          //}
        }
       else{
         window.localStorage['iddept']=  $scope.selectedproductfromtypeahead.deptId;
        productService.setselectedCategory($scope.selectedproductfromtypeahead.category);
       
        //for(var j=0;j<$scope.allitems.length;j++){
        //if($scope.allitems[j].upc==$scope.selectedproductfromtypeahead.upc){
              $scope.screen4 = $scope.selectedproductfromtypeahead.product;
               $scope.iddept=$scope.selectedproductfromtypeahead.deptId;;
              $scope.itemNum=$scope.selectedproductfromtypeahead.upc;
              $scope.Namedept=""
              
            var data ={
            "deptId":$scope.iddept,
            "itemName":$scope.itemNum,
            "productName":$scope.screen4,
            "status":"plus"            
             }
              var results=$filter('filter')($scope.productList,{itemName:data.itemName}, true);
                 if(results.length>0){
                   alert("Product already Added");
                 }
                 else{
                  $scope.productList.push(data);
                   $scope.selectedProduct(data);
                 }
           
         }
       }


    $scope.getallitems=function(){
      dashBoardService.getallitems().then(function (response) {
         $scope.allitems=response.data;
       },function(response){
       });
    }

    $scope.getallitemsfordistributor=function(){
      productService.getItemsforDistributor().then(function (response) {
         $scope.allitems=response.data;
        },function(response){       
        });
    }
 
    $scope.getDepartmentList=function(){
          $scope.productsspinner=true;
          $scope.calldepartment=function(){
             $scope.storeid=dashBoardService.getstoreid();
              //console.log("store is saved....",$scope.storeid);
             if($scope.storeid!=null&&$scope.storeid!=undefined && $scope.storeid!=""&&$scope.storeid!="ALL STORES"){
                  var salesDatafilter={
                      "aggTimeUnit":"1d",
                      "startTime":$scope.SalesDataReportstartDate,
                      "endTime":$scope.Reportenddate,
                      "bucketLevel" : "L",
                      "bucketType":"deptId"
                     }
              productService.getsalesdatabyfilterbystore(salesDatafilter).then(function(response){
                 $scope.departmentList=[{"departmentDescription":"ALL DEPARTMENT"}];
                 for(var i=0;i<response.data.data.length;i++){
                      var results = $filter('filter')($scope.alldepartmentList,{departmentNumber : parseInt(response.data.data[i].id)}, true);
                      if(results.length>0){
                         var deptobj={
                           "departmentDescription":results[0].departmentDescription,
                           "departmentNumber":parseInt(response.data.data[i].id)
                          }
                         $scope.departmentList.push(deptobj);
                        }
                      }
                 $scope.selecteddepartment=$scope.departmentList[0];
                 console.log("departmentList",$scope.departmentList);
              }, function (response) {
                 console.log(response);
              }
             );
          }
         else{
             var salesDatafilter={
                   "aggTimeUnit":"1d",
                   "startTime":$scope.SalesDataReportstartDate,
                   "endTime":$scope.Reportenddate,
                   "bucketLevel" : "L",
                   "bucketType":"deptId"
                   }
              productService.getsalesdatabyfilter(salesDatafilter).then(function(response){
                 $scope.departmentList=[{"departmentDescription":"ALL DEPARTMENT"}];
                     if(response.data.data==undefined){
                         $scope.productsspinner=false;
                         $scope.productchartdata=[];
                         $scope.totalnoOfProducts=0;
                         $scope.masked=false;
                       }
                 for(var i=0;i<response.data.data.length;i++){
                    var results = $filter('filter')($scope.alldepartmentList,{departmentNumber : parseInt(response.data.data[i].id)}, true);
                      if(results.length>0){
                        var deptobj={
                          "departmentDescription":results[0].departmentDescription,
                          "departmentNumber":parseInt(response.data.data[i].id)
                        }
                       $scope.departmentList.push(deptobj);
                     }
                  }
                  $scope.selecteddepartment=$scope.departmentList[0];
                 // $scope.getAllCategories();
             }, function (response) {
                console.log(response);
             });
           }
         }

         $scope.alldepartmentList=[];
         dashBoardService.GetDepartmentList().then(function (response) {
             for(var i=0;i<response.data.length;i++){
                 $scope.alldepartmentList.push(response.data[i]);
             }
            $scope.calldepartment();
         }, function (response) {
            console.log(response);
         });
    }


     $scope.optionid;
     $scope.category;
     $scope.categoryalreadyselected;
     $scope.BrandIdsList;

      $scope.selectdropdown = function () {
         $scope.spin=true;
        if($scope.role=="retailer"){
        $scope.optionpname   = window.localStorage['pname'];
        $scope.option = window.localStorage['selectoption1'];
        $scope.optionid = window.localStorage['iddept'];
         $scope.category= productService.getCategoryforRetailer();
        // console.log("category1235878", $scope.category," ",  $scope.optionid);
        $scope.xy = {
          "Name": $scope.option   
        }

        for(var i=0;i<$scope.ListdeptProduct.length;i++){
          if($scope.ListdeptProduct[i].Name==$scope.option){
          $scope.selectedvalue1=$scope.ListdeptProduct[i];
          }
        }
       // $scope.selectedvalue1 = $scope.xy.Name;
        $scope.subproductList($scope.optionid,$scope.category);
           }
           else if($scope.role=="cpg"){
            // $scope.categoryalreadyselected=window.localStorage['categoryselected'];
            $scope.categoryalreadyselected=productService.getselectedCategory();
           // console.log("categoryalreadyselected", $scope.categoryalreadyselected);
            // if($scope.categoryalreadyselected!=""&&$scope.categoryalreadyselected!=undefined){
                for(var i=0;i<$scope.categoriesList.length;i++){
                if($scope.categoryalreadyselected==$scope.categoriesList[i].category_description){
                $scope.selectedcategory=$scope.categoriesList[i];
                }
              }
              $scope.onlycategory= window.localStorage['onlycategory'];
              if($scope.onlycategory=="true"){
                 $scope.subproductListbycpg($scope.categoryalreadyselected);
                window.localStorage['onlycategory']=false;
              }
              else{
                 $scope.individualcategoryproduct=true;
                 $scope.subproductListbycpg($scope.categoryalreadyselected);
                 window.localStorage['categoryselected']="";
              }
            // }
            // else{
            //   $scope.categoryalreadyselected=$scope.selectedcategory.category_description;
            //   $scope.subproductListbycpg($scope.categoriesList[0].category_description);
            // }

           }
           else {
            // $scope.categoryalreadyselected=window.localStorage['categoryselected'];
              $scope.categoryalreadyselected=productService.getselectedCategory();
  // if($scope.categoryalreadyselected!=""&&$scope.categoryalreadyselected!=undefined){
                for(var i=0;i<$scope.categoriesList.length;i++){
                if($scope.categoryalreadyselected==$scope.categoriesList[i].category_description){
                $scope.selectedcategory=$scope.categoriesList[i];
                }
              }
              $scope.onlycategory= window.localStorage['onlycategory'];
              if($scope.onlycategory=="true"){
                if($scope.selectedBrand.brand_name=="ALL BRANDS"){
                $scope.subproductListbyDistributor($scope.categoryalreadyselected,$scope.BrandIdsList);
                
                }
                else{
                $scope.subproductListbyDistributor($scope.categoryalreadyselected,[$scope.selectedBrandId]);
                }
                window.localStorage['onlycategory']=false;
              }
              else{
                 $scope.individualcategoryproduct=true;
              if($scope.selectedBrand.brand_name=="ALL BRANDS"){
                $scope.subproductListbyDistributor($scope.categoryalreadyselected,$scope.BrandIdsList);
                }
                else{
                $scope.subproductListbyDistributor($scope.categoryalreadyselected,[$scope.selectedBrandId]);
                }                 window.localStorage['categoryselected']="";
              }
            // }
            // else{
            //   $scope.categoryalreadyselected=$scope.selectedcategory.category_description;
            //    if($scope.selectedBrand.brand_name=="ALL BRANDS"){
            //     $scope.subproductListbyDistributor($scope.categoriesList[0].category_description,$scope.BrandIdsList);
            //     }
            //     else{
            //     $scope.subproductListbyDistributor($scope.categoriesList[0].category_description,[$scope.selectedBrandId]);
            //     }
            // }
           }
        $scope.dataplusmark=window.localStorage['dataplus'];
        $scope.optionpitemNumber=window.localStorage['itemNum'];
        var data ={
         // "status":"plus",
       //   "itemName":$scope.optionpitemNumber,
          "deptId":"100",
          "itemName":"00000000000437",
          "productName":"CRV",
          "status":"plus"
        }
      }

        $scope.getcategoriesList=function(){
         productService.getcategoriesListforcpg().then(function (response) {
             $scope.categoriesList=response.data;
            // console.log("categories List....",$scope.categoriesList);
             $scope.selectedcategory=$scope.categoriesList[0];
             $scope.selectdropdown();
           }, function (response) {
             console.log(response);
           }
           );
        }

      $scope.selectData = function (selecteddept) {
        $scope.optionpname   = window.localStorage['pname'];
        $scope.option = window.localStorage['selectoption1'];
        $scope.optionid = window.localStorage['iddept'];
        $scope.value = document.getElementById("mySelect").value;
       // $scope.split = $scope.value.split("|");
        $scope.deptName = selecteddept.Name;
        $scope.deptid =selecteddept.id;
        //console.log("selected department...",selecteddept);
        //console.log($scope.deptid);
        $scope.selectedvalue1=selecteddept;
        $scope.dataDepartmentProduct = $scope.selectedvalue1;
        $scope.selectedItemsDataAraayforbarchart=[];
        $scope.selectedItemsDataAraayforlinechart=[];
        $scope.itemsList=[];
        //console.log($scope.dataDepartmentProduct);
        $scope.option = $scope.deptName;
       // $scope.option1 = dpname;
           if($scope.role=="retailer"){
          $scope.subproductList($scope.deptid);
           }
           else if($scope.role=="cpg"){
            $scope.subproductListbycpg($scope.deptid);
           }
      }


      $scope.categorycallback=function(selectedcategory){

       $scope.selectedItemsDataAraayforbarchart=[];
       $scope.selectedItemsDataAraayforlinechart=[];
       $scope.itemsList=[];
       $scope.individualcategoryproduct=false;
        //console.log("category coming...",selectedcategory);
        $scope.selectedcategory=selectedcategory;
        $scope.categoryalreadyselected=$scope.selectedcategory.category_description;
         if($scope.role=="cpg"){
          $scope.subproductListbycpg($scope.categoryalreadyselected);
         }
         else{
           if($scope.selectedBrand.brand_name=="ALL BRANDS"){
        $scope.subproductListbyDistributor($scope.categoryalreadyselected,$scope.BrandIdsList);
           }
           else{
        $scope.subproductListbyDistributor($scope.categoryalreadyselected,[$scope.selectedBrandId]);
           }
         }
      }

      $scope.subproductList = function (deptid, category) {
      $scope.deptmentid = deptid;
      $scope.categoryName = category;
          sessionStorage.productsSize=1000;
          $scope.getdata;
  
       var deptdataRT =
          {
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {        
            "item.deptId": $scope.deptmentid,
            "item.categoryDesc": $scope.categoryName 
            }
          }
       var deptdataRT1 =
          {
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {        
            "item.deptId": $scope.deptmentid
            //"item.categoryDesc": $scope.categoryName 
            }
          }
        if($scope.categoryName=== "" || $scope.categoryName=== null){
           $scope.getdata = deptdataRT1;
         }else{
           $scope.getdata = deptdataRT;
         }
          productService.RetailerProductbyDepartment($scope.getdata).then(function (response) {
              $scope.subproductlists = [];
              $scope.productList = [];
              for (var i = 0; i < response.data.data.length; i++) {

                 if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
                   var object = {
                    "itemName": response.data.data[i].upc,
                    "name": $scope.productName,
                    "deptId": response.data.data[i].deptId,
                    "amt": response.data.data[i].amt,
                    // "cp": response.data.data[i].cp,
                    // "namecp": response.data.data[i].name
                  };
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
                  $scope.productList.push(productobject);
                  $scope.subproductlists.push(object);
              }

               //console.log("productList",$scope.productList);
              //by default seleting fisrt product
            $scope.flagcheck=window.localStorage['checkflag'];
           $scope.individualproductflag= window.localStorage['individualProduct']
          // for(var i=0; i< $scope.getcompproduct.length; i++){
            if($scope.flagcheck=='screen3'){
              if($scope.getcompproduct == null){
                 $scope.selectedProduct($scope.productList[0]);
               }else if($scope.getcompproduct.length == 2){
               for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else{
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                } 
                 for(var i=0;i<$scope.getcompproduct.length; i++){ 
                $scope.selectedProduct($scope.getcompproduct[i]);
                }    
               }
            }
            else if($scope.individualproductflag=="true") {
              $scope.clickedproductchartfun();
              window.localStorage['individualProduct']=false;
            }
            else{
            if($scope.getcompproduct == null){
                 $scope.selectedProduct($scope.productList[0]);
            }else if($scope.getcompproduct.length == 2){ 
            for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else{
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                }               
            for(var i=0;i<$scope.getcompproduct.length; i++){
            console.log("$scope.getcompproduct",$scope.getcompproduct.length); 
            $scope.selectedProduct($scope.getcompproduct[i]);
            }
               }
            }
             // }
            $scope.addsignlenth = $scope.subproductlists.length;
            $scope.spin=false;
            }, function (response) {
              console.log(response);
            }
          );
      }

      $scope.checkbeforeinsert = function(productobject){
        var flag =  true;
       $scope.productList.forEach(function (item) {
          if(item.itemName === productobject.itemName){
            flag = false;
          }
      });
       return flag;
    }
	
	   $scope.subproductListbycpg = function (deptid) {
      sessionStorage.itemssize=500;
        $scope.deptmentid = deptid;
         $scope.getdata;
          var deptdataRT ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
                "items.categoryDesc" : [deptid],
                "retailerId" :$scope.RetailerIds
               // "items.mfgId" : [$scope.mfgId]
            }
         }
          var deptdataRT1 ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
               "retailerId" :$scope.RetailerIds
            //     "items.categoryDesc" : [deptid]
            //    // "items.mfgId" : [$scope.mfgId]
             }
         }
          if(deptid!=""&&deptid!=undefined){
           $scope.getdata = deptdataRT;
         }else{
           $scope.getdata = deptdataRT1;
         }
          productService.getitemsbycategory($scope.getdata).then(function (response) {
              $scope.subproductlists = [];
              $scope.productList = [];
              for (var i = 0; i < response.data.data.length; i++) {

          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
                  var object = {
                    "itemName": response.data.data[i].upc,
                    "name": $scope.productName,
                    "deptId": response.data.data[i].deptId,
                    "amt": response.data.data[i].amt,
                    "cp": response.data.data[i].cp,
                    "namecp": response.data.data[i].name,
                    "category":deptid
                  };
                  var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus",
                     "category":deptid
                  }
                 if($scope.checkbeforeinsert(productobject)){
                     $scope.productList.push(productobject);
                 }
                  $scope.subproductlists.push(object);
              }
            // for(var i=0; i< $scope.getcompproduct.length; i++){
              if($scope.individualcategoryproduct==true){
               $scope.optionpname   = window.localStorage['pname'];
               $scope.option = window.localStorage['selectoption1'];
               $scope.optionid = window.localStorage['iddept'];
               $scope.dataplusmark=window.localStorage['dataplus'];
               $scope.optionpitemNumber   =  window.localStorage['itemNum'];
                 var data ={
                  "deptId":$scope.optionid,
                  "itemName":$scope.optionpitemNumber,
                  "productName":$scope.optionpname,
                  "status":"plus",
                  "category":deptid
                 }
                 //console.log("product list...",$scope.productList);
                 //console.log("compare product...",$scope.getcompproduct);
                /* if($scope.checkbeforeinsert(data)){
                  $scope.productList.push(data);
                 }*/
                for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else{
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                }
                if($scope.getcompproduct == null){
                     $scope.selectedProduct(data);
                 }else if($scope.getcompproduct.length == 2){
                  for(var i=0;i<$scope.getcompproduct.length;i++){
                  $scope.selectedProduct($scope.getcompproduct[i]);
                }
               }
              }
              else{
                 if($scope.getcompproduct == null){
                     $scope.selectedProduct($scope.productList[0]);
                 }else if($scope.getcompproduct.length == 2){
                   for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else{
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                }
                  for(var i=0; i< $scope.getcompproduct.length; i++){
                     $scope.selectedProduct($scope.getcompproduct[i]);
                   }
                 }
              }
            }, function (response) {
              console.log(response);
            }
          );
      }
	  
	  
           $scope.clickedproductchartfun=function () {
             $scope.s2status=window.localStorage['s2'];
             if($scope.s2status=='true'){
               $scope.optionpname   = window.localStorage['pname'];
               $scope.option = window.localStorage['selectoption1'];
               $scope.optionid = window.localStorage['iddept'];
               $scope.dataplusmark=window.localStorage['dataplus'];
               $scope.optionpitemNumber   =  window.localStorage['itemNum'];
          var data ={
            "deptId":$scope.optionid,
            "itemName":$scope.optionpitemNumber,
            "productName":$scope.optionpname,
            "status":"plus"
             }
             $scope.selectedProduct(data);
             }
           }

      $scope.option = window.localStorage['selectoption1'];
       $scope.List = [
           {
            "store_id":"",
            "store_name":"ALL STORES"
           }
           ];

      $scope.GetStoreList=function(){
      dashBoardService.GetStoreList().then(function (response) {
          for (var i = 0; i < response.data.length; i++) {
            $scope.List.push(response.data[i]);
          }
          $scope.selectedOption = $scope.List[0];

          $scope.storeid=dashBoardService.getstoreid();
          for(var j=0;j<$scope.List.length;j++){
            if($scope.List[j].store_id==$scope.storeid){
             $scope.selectedOption = $scope.List[j];
              $scope.selectedstoreidname=$scope.List[j].store_name;
            }
          }
        }, function (response) {
        }
      );
    }


      $scope.productList1 = [];
      $scope.subproductlists1 = [];
      $scope.Storesubproductlists1 = [];
      $scope.subproductlistsline=[];
      $scope.subproductlistsline1=[];

      $scope.getjatStyle = function (value) {
        if (value >= 1) {
          return "page_dashboard_box_index green";
        } else {
          return "page_dashboard_box_index red";
        }
      };


      $scope.getindexStyle = function (value) {
        
        if (value >= 1) {
          return "product-text-arrow";
        } else {
          return "product-text-arrowred";
        }
      };


       $scope.getarrowStyle = function (value) {
        if (value >= 1) {
          return "glyphicon glyphicon glyphicon-arrow-up";
        } else {
          return "glyphicon glyphicon glyphicon-arrow-down";
        }
      };

      $scope.signvalue = "+";

      var data={

        "itemName":$scope.itemNum=window.localStorage['itemNum']
      }
$scope.upcDataArr = [];
$scope.avgbasketArr=[];
 
 $scope.avgBasketByRT = function (itemNumber, avgBasketTotalByCP) {
    console.log("upc....",itemNumber);

    var ShoppingTripsdataRT = {
      "aggTimeUnit": "1d",
           "startTime":$scope.ReportstartDate,
            "endTime":$scope.Reportenddate,
           "filters": {
            "item.upc" : [itemNumber]
        }
       }
       dashBoardService.GetAvgBasket(ShoppingTripsdataRT).then(function (response) {
        $scope.avgBasketTotal = response.data.stats.avg;
          $scope.avgBasketTotalByRT = parseFloat(response.data.stats.avg);
          if (avgBasketTotalByCP == 0 ||avgBasketTotalByCP == null || isNaN(avgBasketTotalByCP)) {
            $scope.ABtotal = 0;
             // $scope.avgbasketArr.push($scope.ABtotal);
              $scope.upcDataArr.push({ABtotal:$scope.ABtotal, UPC:itemNumber});
          }
          else {
            $scope.ABtotal = $scope.avgBasketTotalByRT / avgBasketTotalByCP;
            $scope.ABtotal = $scope.ABtotal.toFixed(2);
            // $scope.avgbasketArr.push($scope.ABtotal);
            $scope.upcDataArr.push({ABtotal:$scope.ABtotal, UPC:itemNumber });
          }
           
           for(var i=0;i<$scope.itemsList.length;i++){
            for(var j=0;j< $scope.upcDataArr.length; j++){
               if($scope.itemsList[i].itemNumber === $scope.upcDataArr[j].UPC){
                  $scope.itemsList[i].linechart.avgbasket=$scope.upcDataArr[j].ABtotal;
                  $scope.itemsList[i].barchart.avgbasket=$scope.upcDataArr[j].ABtotal;
               }
           }
         }
     // console.log( " $scope.upcDataArr",$scope.upcDataArr);

      }, function (response) {
        console.log(response);
      }
      );
   
}


$scope.avgBasketFunction = function (upc) {
  // $scope.upcDataArr.push({UPC:upc});
        var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "filters": {
            "item.upc" : [upc]
          
        }
        }
        dashBoardService.GetAvgBasket(ShoppingTripsdata).then(function (response) {
          
           if(response.data.stats){
          $scope.avgBasketTotalByCP = parseFloat(response.data.stats.avg);

           }
            $scope.avgBasketByRT(upc, $scope.avgBasketTotalByCP);
      
        }, function (response) {
          console.log(response);
        }
        );
       }
          
  

  
    $scope.getbasketforcpgstore=[];
    $scope.avgBasketByRTbystore = function (itemNumber,avgBasketTotalByCP) {
    var ShoppingTripsdataRT = {
           "aggTimeUnit": "1d",
           "startTime":$scope.ReportstartDate,
            "endTime":$scope.Reportenddate,
           "filters": {
            "item.upc" : [itemNumber]
        }

       }
       
         dashBoardService.GetAvgBasketByStoreId(ShoppingTripsdataRT).then(function (response) {

        $scope.avgBasketTotal = response.data.stats.avg;

          $scope.avgBasketTotalByRT = parseFloat(response.data.stats.avg);

          if (avgBasketTotalByCP == 0 ||avgBasketTotalByCP == null || isNaN(avgBasketTotalByCP)) {
            $scope.ABtotal = 0;
             $scope.getbasketforcpgstore.push({UPC:itemNumber, ABtotal:$scope.ABtotal});
          }
          else {
            if($scope.avgBasketTotalByRT){
            $scope.ABtotal = $scope.avgBasketTotalByRT /avgBasketTotalByCP;
            $scope.ABtotal = $scope.ABtotal.toFixed(2);
            $scope.getbasketforcpgstore.push({UPC:itemNumber, ABtotal:$scope.ABtotal});
          }
           else{
               $scope.ABtotal = 0.00;
               $scope.ABtotal = $scope.ABtotal.toFixed(2);
               $scope.getbasketforcpgstore.push({UPC:itemNumber, ABtotal:$scope.ABtotal});
            }
          }

          for(var i=0;i<$scope.itemsList.length;i++){
            for(var j=0;j<$scope.getbasketforcpgstore.length; j++){
              if($scope.itemsList[i].itemNumber===$scope.getbasketforcpgstore[j].UPC){
                $scope.itemsList[i].linechart.avgbasket=$scope.getbasketforcpgstore[j].ABtotal;
                $scope.itemsList[i].barchart.avgbasket=$scope.getbasketforcpgstore[j].ABtotal;
            }
          }
         }

      }, function (response) {
        console.log(response);
      }
      );
   }

    $scope.avgBasketFunctionbystore = function (upc) {
   var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "filters": {
            "item.upc" : [upc]
          
        }
        }
        dashBoardService.GetAvgBasketByStoreId(ShoppingTripsdata).then(function (response) {
          if(response.data.stats){
              $scope.avgBasketTotalByCP = parseFloat(response.data.stats.avg);
           }         
           $scope.avgBasketByRTbystore(upc, $scope.avgBasketTotalByCP);

         }, function (response) {
          console.log(response);
        }
        );
    }

    $scope.ShoppingtripArr=[];
     $scope.ShoppingTripsbyRT = function (itemnumber,ShoppingTripsCPindex) {
        var ShoppingTripsdataRT = {
          "aggTimeUnit": "1d",
         "startTime":$scope.ReportstartDate,
          "endTime":$scope.Reportenddate,
          "filters": {
           "terms" : {
            "item.upc" : [itemnumber]
          }
        }
       }

       dashBoardService.GetShoppingTrips(ShoppingTripsdataRT).then(function (response) {
        $scope.ShoppingTripsTotalbyRT = response.data.total;
        $scope.ShoppingTripsRTindex = parseFloat(response.data.total);

                if (ShoppingTripsCPindex == 0 || ShoppingTripsCPindex == null || isNaN(ShoppingTripsCPindex)) {
                  $scope.STIndex = 0;
                   // $scope.ShoppingtripArr.push($scope.STIndex);
                    $scope.ShoppingtripArr.push({STIndex:$scope.STIndex, UPC:itemnumber });
                }
                else {
                  $scope.STIndex = $scope.ShoppingTripsRTindex /ShoppingTripsCPindex;
                  $scope.STIndex = $scope.STIndex.toFixed(2);
                  // $scope.ShoppingtripArr.push($scope.STIndex);
                  $scope.ShoppingtripArr.push({STIndex:$scope.STIndex, UPC:itemnumber });
                }
          for(var i=0;i<$scope.itemsList.length;i++){
            for(var j=0; j<$scope.ShoppingtripArr.length; j++){
          if($scope.itemsList[i].itemNumber === $scope.ShoppingtripArr[j].UPC){
            $scope.itemsList[i].linechart.shoppingtrips=$scope.ShoppingtripArr[j].STIndex;
            $scope.itemsList[i].barchart.shoppingtrips=$scope.ShoppingtripArr[j].STIndex;
           }
          }
         }
            }, function (response) {
              console.log(response);
            }
            );
       }

    $scope.ShoppingTripsFunction = function (upc) {
      // console.log("arr",$scope.upcDataArr);
          var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "filters": {
            "terms" : {
            "item.upc" : [upc]
          }
        }
        }
        dashBoardService.GetShoppingTrips(ShoppingTripsdata).then(function (response) {
          $scope.ShoppingTripsCPindex = parseFloat(response.data.total);
           $scope.ShoppingTripsbyRT(upc,$scope.ShoppingTripsCPindex);
        }, function (response) {
          console.log(response);
        }
        );
    }
  
  $scope.STPbystoreCPG=[];
   $scope.ShoppingTripsbyRTbystore = function (itemnumber,ShoppingTripsCPindex) {
        var ShoppingTripsdataRT = {
          "aggTimeUnit": "1d",
         "startTime":$scope.ReportstartDate,
          "endTime":$scope.Reportenddate,
          "filters": {
           "terms" : {
            "item.upc" : [itemnumber]
          }
        }
       }
       dashBoardService.GetShoppingTripsByStoreId(ShoppingTripsdataRT).then(function (response) {
        $scope.ShoppingTripsTotalbyRT = response.data.total;
                $scope.ShoppingTripsRTindex = parseFloat(response.data.total);
                if (ShoppingTripsCPindex == 0 || ShoppingTripsCPindex == null || isNaN(ShoppingTripsCPindex)) {
                   $scope.STIndex = 0;
                   $scope.STPbystoreCPG.push({UPC:itemnumber, STIndex: $scope.STIndex});
                }
                else {
                  $scope.STIndex = $scope.ShoppingTripsRTindex /ShoppingTripsCPindex;
                  $scope.STIndex = $scope.STIndex.toFixed(2);
                   $scope.STPbystoreCPG.push({UPC:itemnumber, STIndex: $scope.STIndex});
                }
          for(var i=0;i<$scope.itemsList.length;i++){
            for(var j=0; j<$scope.STPbystoreCPG.length; j++){
             if($scope.itemsList[i].itemNumber===$scope.STPbystoreCPG[j].UPC){
            $scope.itemsList[i].linechart.shoppingtrips=$scope.STPbystoreCPG[j].STIndex;
            $scope.itemsList[i].barchart.shoppingtrips=$scope.STPbystoreCPG[j].STIndex;
          }
          }
         }
            }, function (response) {
              console.log(response);
            }
            );
   }
 
    $scope.ShoppingTripsFunctionbystore = function (upc) {
   var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "filters": {
            "terms" : {
            "item.upc" : [upc]
          }
        }
        }
        dashBoardService.GetShoppingTripsByStoreId(ShoppingTripsdata).then(function (response) {
          $scope.ShoppingTripsCPindex = parseFloat(response.data.total);
          $scope.ShoppingTripsbyRTbystore(upc,$scope.ShoppingTripsCPindex);
        }, function (response) {
          console.log(response);
        }
        );
    }

   $scope.upcArr=[];
   $scope.cpgavgbasketArr=[];

   $scope.shareOfCategoryByRT = function (itemnumber,avgBasketTotalByCP) {
            var ShoppingTripsdata = {
              "aggTimeUnit": "1d",
              "startTime":$scope.ReportstartDate,
              "endTime":$scope.Reportenddate,
              "rid" : "000",
              "filters": {
              "terms":{
              "item.upc" : [itemnumber],
              "retailerId" :$scope.RetailerIds
              }}
            }
              $scope.AvgBasket = false;
           productService.getShoppingTripsforDistributor(ShoppingTripsdata).then(function (response) {
          $scope.avgBasketTotal = response.data.total;
          $scope.avgBasketTotalByRT = parseFloat(response.data.total);
          if (avgBasketTotalByCP == 0 || avgBasketTotalByCP == null || isNaN(avgBasketTotalByCP)) {
            $scope.ABtotal = 0;
            $scope.cpgavgbasketArr.push({UPC:itemnumber, ABtotal:$scope.ABtotal});
          }
          else {
            $scope.ABtotal = $scope.avgBasketTotalByRT /avgBasketTotalByCP;
            $scope.ABtotal = $scope.ABtotal.toFixed(2);
             $scope.cpgavgbasketArr.push({UPC:itemnumber, ABtotal:$scope.ABtotal});
          }
          for(var i=0;i<$scope.itemsList.length;i++){
             for(var j=0; j<$scope.cpgavgbasketArr.length; j++){
                if($scope.itemsList[i].itemNumber== $scope.cpgavgbasketArr[j].UPC){
                   $scope.itemsList[i].linechart.shoppingtrips=$scope.cpgavgbasketArr[j].ABtotal;
                   $scope.itemsList[i].barchart.shoppingtrips=$scope.cpgavgbasketArr[j].ABtotal;
            }
           }
         }
            }, function (response) {
              console.log(response);
            }
            );
          }

    $scope.shareOfCategoryByAllRetailers= function (upc) {
      $scope.upcArr.push(upc);
          var ShoppingTripsdata = {
            "aggTimeUnit": "1d",
            "startTime": $scope.ComparestartDate,
            "endTime": $scope.Compareenddate,
            // "rid" : "000",
              "filters": {
                "terms":{
               "item.upc" : [upc],
               "retailerId" :$scope.RetailerIds
              }}
          }
         productService.getShoppingTripsforDistributor(ShoppingTripsdata).then(function (response) {
           $scope.avgBasketTotalByCP = parseFloat(response.data.total);
            $scope.shareOfCategoryByRT(upc,$scope.avgBasketTotalByCP);

          }, function (response) {
            console.log(response);
          }
          );               
      }

     $scope.cpgshoppingtripArr=[];

        $scope.ShareOfBasketByAllRetailerbyRT = function (itemnumber,ShoppingTripsCPindex) {

            var ShoppingTripsdata = {
               "aggTimeUnit": "1d",
               "startTime":$scope.ReportstartDate,
               "endTime":$scope.Reportenddate,
               // "rid" : "000",
               "filters": {
                   "item.upc" : [itemnumber],
                   "retailerId" :$scope.RetailerIds
               }
            }
           productService.getAvgBasketforDistributor(ShoppingTripsdata).then(function (response) {
             $scope.ShoppingTripsTotalbyRT = response.data.stats.avg;
                $scope.ShoppingTripsRTindex = parseFloat(response.data.stats.avg);

                if (ShoppingTripsCPindex == 0 ||ShoppingTripsCPindex == null || isNaN(ShoppingTripsCPindex)) {
                  $scope.STIndex = 0;
                  $scope.cpgshoppingtripArr.push({UPC:itemnumber, STIndex:$scope.STIndex});
                }
                else {
                  $scope.STIndex = $scope.ShoppingTripsRTindex /ShoppingTripsCPindex;
                  $scope.STIndex = $scope.STIndex.toFixed(2);
                   $scope.cpgshoppingtripArr.push({UPC:itemnumber, STIndex:$scope.STIndex});
                }

          for(var i=0; i<$scope.itemsList.length;i++){
            for(var j=0; j<$scope.cpgshoppingtripArr.length; j++){
                if($scope.itemsList[i].itemNumber== $scope.cpgshoppingtripArr[j].UPC){
                   $scope.itemsList[i].linechart.avgbasket=$scope.cpgshoppingtripArr[j].STIndex;
                   $scope.itemsList[i].barchart.avgbasket=$scope.cpgshoppingtripArr[j].STIndex;
           }
          }
         }

            }, function (response) {
              console.log(response);
            }
            );
          }


    $scope.ShareOfBasketByAllRetailer = function (upc) {
      
             var ShoppingTripsdata = {
            "aggTimeUnit": "1d",
            "startTime": $scope.ComparestartDate,
            "endTime": $scope.Compareenddate,
            // "rid" : "000",
            "filters": {
            "item.upc" : [upc],
            "retailerId" :$scope.RetailerIds
        }
          }
           productService.getAvgBasketforDistributor(ShoppingTripsdata).then(function (response) {
          $scope.ShoppingTripsCPindex = parseFloat(response.data.stats.avg);
          $scope.ShareOfBasketByAllRetailerbyRT(upc,$scope.ShoppingTripsCPindex)
          }, function (response) {
            console.log(response);
          }
          );
        }

     var avgBasket;
     avgBasket=$rootScope.$on('avgbasket',function (event, data) {

          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==data.upc){
            $scope.itemsList[i].linechart.avgbasket=data.avgbasket;
            $scope.itemsList[i].barchart.avgbasket=data.avgbasket;
          }
         }
         });

          var shoppingtrips;

         shoppingtrips=$rootScope.$on('shoppingtrips',function (event, data) {
      
          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==data.upc){
            $scope.itemsList[i].linechart.shoppingtrips=data.shoppingtrips;
            $scope.itemsList[i].barchart.shoppingtrips=data.shoppingtrips;

          }
         }

         });


          var salesindex;

         salesindex=$rootScope.$on('spindex',function (event, data) {
      
          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==data.upc){
            $scope.itemsList[i].linechart.index=data.spindex;
            $scope.itemsList[i].linechart.total=data.total;
            $scope.itemsList[i].linechart.unitsold.unitSoldbyRp=data.unitsold;
          }
         }
         });

         var salesindexbarchart;

         salesindexbarchart=$rootScope.$on('spindexbarchart',function (event, data) {
      
          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==data.upc){
            $scope.itemsList[i].barchart.index=data.spindex;
            $scope.itemsList[i].barchart.total=data.total;
            $scope.itemsList[i].barchart.unitsold.unitSoldbyRp=data.unitsold;
          }
         }

         });

       $scope.selectedItemsDataAraayforbarchart=[];
       $scope.selectedItemsDataAraayforlinechart=[];

         $scope.getproductforRT = function (deptmName, productname, unitsoldbycp,spIndextotal) {
         $scope.unitsoldbycp=unitsoldbycp;
         $scope.spIndextotal=spIndextotal;      
         //console.log("getproductforRT",deptmName,"",productname,"",$scope.unitsoldbycp);
         var data = {
                     "aggTimeUnit": "1d",
                     "startTime": $scope.SalesDataReportstartDate,
                     "endTime": $scope.Reportenddate,
                     "filters":{
                     "item.upc": [deptmName]
                     }
                    }

                    $scope.showproducts=false;
                    $scope.spin=true;
                    productService.GetSalesPerformance(data).then(function (response) {
                      $scope.ProductsbyRT = response.data.data;

                      $scope.singleProductchartId = productService.generateguid();
                      $scope.rpIndextotal = parseFloat(response.data.total);
                         
                        $scope.totalunitsold=response.data.totalSoldQty;

                    var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }

                      if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;

                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }

                      $scope.barchartData = [];

                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                     
                      }

                      $scope.barchartData.push(barchartDataObject);

                      var barchartObject = {

                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber":deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }

                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                      $scope.avgBasketFunction(deptmName);
                      $scope.ShoppingTripsFunction(deptmName);
                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];
                var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
                var days = b.diff(a, 'days');
        $scope.j=1;
         for (var i = 0; i <= days; i++) {
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");
               $scope.j++;
             }
             if($scope.ProductsbyCT&&$scope.ProductsbyRT){
              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){
                var linechartdataobject = {

                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName":productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);
                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                        "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                  
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                        "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                        "unitSoldbyRp":0,
                        "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name":  deptmName,
                        "productName": productname,
                        "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){
                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                        "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name":  deptmName,
                        "productName":  productname,
                        "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                        "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
            }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber": deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName": productname,
                              "shoppingtrips":"",
                              "avgbasket":"",
                              "unitsold":unitsold,
                              "arrowdircetion":"glyphicon-arrow-down",
                              "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);
                            var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber": deptmName
                            }
                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                             $scope.$applyAsync();
                            }, function (response) {
                              console.log(response);
                            }
                          );
                         }
       
       $scope.getproductBySelectedItem = function (deptmName,productname) {
                          var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters": {
                          "item.upc": [deptmName]
                          }
                        }
                        productService.GetSalesPerformance(data1).then(function (response) {                          
                            $scope.ProductsbyCT = response.data.data;
                            if(response.data){
                            $scope.unitsoldbycp=response.data.totalSoldQty
                            }
                          $scope.spIndextotal = parseFloat(response.data.total);
                          $scope.getproductforRT(deptmName, productname, $scope.unitsoldbycp, $scope.spIndextotal);
                          }, function (response) {
                            console.log(response);
                          }
                        );
                       // }, 1000);
                      }

        $scope.getproductBySelectedItemforcpg =function(deptmName,productname) {
        $scope.getproductforRTforcpg =function (){
                   var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters": {
                          "item.upc": [deptmName],
                          "retailerId" :$scope.RetailerIds
                          //"items.mfgId" :[$scope.mfgId],
                          //"item.categoryDesc":[$scope.selectedcategory.category_description]
                          }
                        }
                        productService.GetSalesPerformanceByAllRetailerswithoutsize(data1).then(function (response) {
                    $scope.ProductsbyCT = response.data.data;
                    if(response.data){
                    $scope.unitsoldbycp=response.data.totalSoldQty
                    }
                    $scope.spIndextotal = parseFloat(response.data.total);
                    $scope.$applyAsync();
         var data = {
                     "aggTimeUnit": "1d",
                     "startTime": $scope.SalesDataReportstartDate,
                     "endTime": $scope.Reportenddate,
                     "filters":{
                     "item.upc": [deptmName],
                     "retailerId" :$scope.RetailerIds
                     // "items.mfgId" : [$scope.mfgId],
                     // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                     }
                    }
                    $scope.showproducts=false;
                    $scope.spin=true;
                    productService.GetSalesPerformanceByAllRetailerswithoutsize(data).then(function (response) {
                      $scope.ProductsbyRT = response.data.data;
                      $scope.singleProductchartId = productService.generateguid();
                      $scope.rpIndextotal = parseFloat(response.data.total);
                      $scope.totalunitsold=response.data.totalSoldQty;
                        var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }
                      if($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;
                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }
                      $scope.barchartData = [];
                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                      }
                      $scope.barchartData.push(barchartDataObject);
                      var barchartObject = {
                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }
                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                      $scope.shareOfCategoryByAllRetailers(deptmName);
                      $scope.ShareOfBasketByAllRetailer(deptmName);
                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];
                var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
                var days = b.diff(a, 'days');
                $scope.j=1;
                for (var i=0;i<=days;i++){
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
              $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");;
               $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");;
               $scope.j++;
             }
            if($scope.ProductsbyCT&&$scope.ProductsbyRT){
            if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){
            var linechartdataobject = {
                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);
                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                        "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){

                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                              "avgbasket":"",
                              "unitsold":unitsold,
                              "arrowdircetion":"glyphicon-arrow-down",
                              "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);
                            var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }
                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                              $scope.$applyAsync();
                            }, function (response) {
                              console.log(response);
                            }
                          );
                          }, function (response) {
                            console.log(response);
                          }
                        );
                        }
                            $scope.getproductforRTforcpg();
                      }

   $scope.getproductBySelectedItemforcpgbyretailer=function(deptmName,productname) {
  $scope.getproductforRTforcpgbyretailer = function () {
          var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters":{
                     "item.upc": [deptmName]
                     // "items.mfgId" : [$scope.mfgId],
                     // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                     }
                        }
  productService.GetSalesPerformance(data1).then(function (response) {
        $scope.ProductsbyCT = response.data.data;
        if(response.data){
          $scope.unitsoldbycp=response.data.totalSoldQty
        }
        $scope.spIndextotal = parseFloat(response.data.total);
         var data = {
                     "aggTimeUnit": "1d",
                     "startTime": $scope.SalesDataReportstartDate,
                     "endTime": $scope.Reportenddate,
                    "filters":{
                     "item.upc": [deptmName]
                     // "items.mfgId" : [$scope.mfgId],
                     // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                     }
                    }
                    $scope.showproducts=false;
                    $scope.spin=true;
                    productService.GetSalesPerformance(data).then(function (response) {
                      $scope.ProductsbyRT = response.data.data;
                      $scope.singleProductchartId = productService.generateguid();
                      $scope.rpIndextotal = parseFloat(response.data.total);
                      $scope.totalunitsold=response.data.totalSoldQty;
                        var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }
                      if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;
                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }
                      $scope.barchartData = [];
                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                      }
                      $scope.barchartData.push(barchartDataObject);
                      var barchartObject = {
                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }
                     $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                     $scope.avgBasketFunction(deptmName);
                     $scope.ShoppingTripsFunction(deptmName);
                     $scope.singleProductLinechartId = productService.generateguid();
                     $scope.linechartData = [];
          var a = moment($scope.ReportstartDate);
          var b = moment($scope.Reportenddate);
          var days = b.diff(a, 'days');
        $scope.j=1;
         for (var i = 0; i <= days; i++) {
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");
               $scope.j++;
             }
             if($scope.ProductsbyCT&&$scope.ProductsbyRT){
              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){
                var linechartdataobject = {
                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);
                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                       "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){

                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                               "avgbasket":"",
                               "unitsold":unitsold,
                               "arrowdircetion":"glyphicon-arrow-down",
                               "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);

                        var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }

                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;

                            }, function (response) {
                              console.log(response);
                            }
                          );
                            $scope.$applyAsync();
                          }, function (response) {
                            console.log(response);
                          }
                        );
                        }
                        $scope.getproductforRTforcpgbyretailer();
                      }
                   $scope.selectedItemsArray=[];
                   $scope.linechartapicallstatus=false;
                   $scope.barchartapicallstatus=false;
                 $scope.getUPC=[];
                 $scope.getprdctName=[];
            $scope.selectedProduct = function (data) {
                     $scope.masked = true;
                     // $scope.selecteditem=[];                     
                    //console.log("data",data);
                       // $scope.selectitem.push({productName: data.productName,itemName:data.itemName});
                       // console.log("$scope.selectitem",$scope.selectitem);
                     if(data){
                         $scope.getUPC.push(data.itemName);
                         $scope.getprdctName.push(data.productName);
                         $scope.selectedproductName = data.productName;
                         $scope.selecteditem=data.itemName;
                // console.log("selecteditem",$scope.selecteditem);
                      $scope.selectedItemsArray.push($scope.selecteditem);
                         if (data.status == "plus") {
          for (var i =0;i<$scope.productList.length;i++) {
          if ($scope.productList[i].itemName == data.itemName) {
              $scope.productList[i].status = "minus"
          }
          }
        if($scope.role=="retailer"){
          $scope.storeid=dashBoardService.getstoreid();
          if($scope.storeid!=null&&$scope.storeid!=undefined&&
            $scope.storeid!=""&&$scope.storeid!="ALL STORES"){
             $scope.getselectedproductsbyStoreId($scope.selecteditem,$scope.selectedproductName);
          }
        else{
        $scope.getproductBySelectedItem($scope.selecteditem,$scope.selectedproductName);
        }
       }
      else if($scope.role=="distributor"){
              /* if($scope.selectedBrand.brand_name=="ALL BRANDS"){
               }
               else{
               }*/
               $scope.getproductBySelectedItemforDistributor($scope.selecteditem,$scope.selectedproductName);
              }
              else if($scope.role=="cpg"){
              $scope.selectedcpg=dashBoardService.getsavestoreselected();
              $scope.selecteddma=dashBoardService.getselectedDMA();
               if($scope.selecteddma !== null ){
                 $scope.dma= $scope.selecteddma.name;
                  $scope.dmaStoreList=[];
                  for(var i=0;i<$scope.selecteddma.stores.length;i++){
                  $scope.dmaStoreList.push($scope.selecteddma.stores[i].toString());
                  }
                  $scope.getproductBySelectedItemforDMA($scope.selecteditem,$scope.selectedproductName);
                  }
          else{ 
         if($scope.selectedcpg.name=="All Retailers"){
          $scope.getproductBySelectedItemforcpg($scope.selecteditem,$scope.selectedproductName)
         }
         else{
          if($scope.selectedcpg.hasOwnProperty("children")){
            sessionStorage.user=$scope.selectedcpg.retailerId;
            $scope.retailerid=$scope.selectedcpg.retailerId;
              $scope.getproductBySelectedItemforcpgbyretailer($scope.selecteditem,$scope.selectedproductName)
          }
          else{
             sessionStorage.user=$scope.selectedcpg.retailerId;
            $scope.storeId = $scope.selectedcpg.storeid;
            sessionStorage.storeId = $scope.storeId;
          $scope.getselectedproductsbyStoreIdforcpg($scope.selecteditem,$scope.selectedproductName)
        }
        }
      }
        }
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=true;
        }
       else{
                              //console.log("else inside ",data);
          for(var i=0;i<$scope.productList.length;i++) {
          if($scope.productList[i].itemName==data.itemName) {
                    $scope.productList[i].status = "plus"
          }
        }
        for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
        if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==data.itemName) {
        $scope.selectedItemsDataAraayforbarchart.splice(i, 1);
        }
      }

    for(var i=0;i<$scope.selectedItemsDataAraayforlinechart.length;i++){
    if($scope.selectedItemsDataAraayforlinechart[i].itemNumber==data.itemName) {
      $scope.selectedItemsDataAraayforlinechart.splice(i, 1);
    }
    }

    for(var j=0;j<$scope.itemsList.length;j++){
    if($scope.itemsList[j].itemNumber==data.itemName){
        $scope.itemsList.splice(j, 1);
    }
    }
        $scope.masked = false;
    }
    }  
    else{
      $scope.masked = false;
    }
    }//end of selected product function...

                    $scope.getselectedproductsbyStoreId =function(deptmName,productname) {
                        $scope.getproductforRT = function () {

                          var data = {
                            "aggTimeUnit": "1d",
                            "startTime": $scope.SalesDataReportstartDate,
                            "endTime": $scope.Reportenddate,
                            "filters": {
                              "item.upc": [deptmName]
                            }
                          }

                      //console.log("deptmName in getselectedproductsbyStoreId",data);

                          $scope.showproducts=false;
                          $scope.spin=true;
                          productService.GetSalesPerformanceByStoreId(data).then(function (response) {
                          $scope.ProductsbyRT = response.data.data;

                      $scope.singleProductchartId = productService.generateguid();
                      $scope.rpIndextotal = parseFloat(response.data.total);
                         
                        $scope.totalunitsold=response.data.totalSoldQty;

                         var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }

                      if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;

                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }

                      $scope.barchartData = [];

                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#ba5bbb",
                        "color1": "#428DB6",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                     
                      }

                      $scope.barchartData.push(barchartDataObject);

                      var barchartObject = {

                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }

                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);

                     $scope.avgBasketFunctionbystore(deptmName);
                     $scope.ShoppingTripsFunctionbystore(deptmName);
                     // console.log("push in toarray", $scope.selectedItemsDataAraayforbarchart);
                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];

 var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
              var days = b.diff(a, 'days');

        $scope.j=1;
      
         for (var i = 0; i <= days; i++) {

             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");

               $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");

             }
             else{
               
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");
          $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");

               $scope.j++;

             }
if($scope.ProductsbyCT&&$scope.ProductsbyRT){

              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){

                var linechartdataobject = {

                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);

                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                  
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                       "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){
                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                               "avgbasket":"",
                               "unitsold":unitsold,
                               "arrowdircetion":"glyphicon-arrow-down",
                               "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);
                           var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }
                          $scope.itemsList.push(objectitems);
                          $scope.showproducts=true;
                          $scope.spin=false;
                          $scope.masked = false;
                            }, function (response) {
                              console.log(response);
                            }
                          );
                        }
                        var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters": {
                            "item.upc": [deptmName]
                          }
                        }

                        productService.GetSalesPerformanceByStoreId(data1).then(function (response) {
                            $scope.ProductsbyCT = response.data.data;
                            if(response.data){
                              $scope.unitsoldbycp=response.data.totalSoldQty
                            }
                          $scope.spIndextotal = parseFloat(response.data.total);
                           
                          }, function (response) {
                            console.log(response);
                          }
                        );
                         $scope.getproductforRT();
                    }


                    $scope.getselectedproductsbyStoreIdforcpg =function(deptmName,productname) {
                        $scope.getproductforRTforcpgbystore = function () {
                          var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                           "filters": {
                            "item.upc": [deptmName]
                            // "items.mfgId" : [$scope.mfgId],
                            // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                          }
                        }

                        productService.GetSalesPerformanceByStoreId(data1).then(function (response) {

                            $scope.ProductsbyCT = response.data.data;
                            if(response.data){
                              $scope.unitsoldbycp=response.data.totalSoldQty
                            }
                          $scope.spIndextotal = parseFloat(response.data.total);

                          var data = {
                            "aggTimeUnit": "1d",
                            "startTime": $scope.SalesDataReportstartDate,
                            "endTime": $scope.Reportenddate,
                            "filters": {
                            "item.upc": [deptmName]
                            // "items.mfgId" : [$scope.mfgId],
                            // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                          }
                          }

                          $scope.showproducts=false;
                          $scope.spin=true;
                          productService.GetSalesPerformanceByStoreId(data).then(function (response) {
                          $scope.ProductsbyRT = response.data.data;
                          $scope.singleProductchartId = productService.generateguid();
                          $scope.rpIndextotal = parseFloat(response.data.total);
                          $scope.totalunitsold=response.data.totalSoldQty;
                         var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }
                      if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;
                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }

                      $scope.barchartData = [];

                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#ba5bbb",
                        "color1": "#428DB6",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                     
                      }

                      $scope.barchartData.push(barchartDataObject);
                      var barchartObject = {
                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }

                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                     // $scope.avgBasketFunctionbystoreforCPG(deptmName);
                     // $scope.ShoppingTripsFunctionbystoreforCPG(deptmName);
                     $scope.ShoppingTripsFunctionbystore(deptmName);
                      $scope.avgBasketFunctionbystore(deptmName);
                     // console.log("push in toarray", $scope.selectedItemsDataAraayforbarchart);
                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];

 var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
              var days = b.diff(a, 'days');

        $scope.j=1;

         for (var i = 0; i <= days; i++) {
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");
               $scope.j++;
             }

             if($scope.ProductsbyCT&&$scope.ProductsbyRT){

              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){

                var linechartdataobject = {

                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);

                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                  
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                       "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }

                      $scope.linechartData.push(linechartdataobject);

                    }

                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }

                      $scope.linechartData.push(linechartdataobject);

                    }

              }
              else{
                if($scope.ProductsbyRT[i]){

                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                               "avgbasket":"",
                               "unitsold":unitsold,
                               "arrowdircetion":"glyphicon-arrow-down",
                               "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);

                         var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }
                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                            }, function (response) {
                              console.log(response);
                            }
                          );

                          }, function (response) {
                            console.log(response);
                          }
                        );
                      }
                        $scope.getproductforRTforcpgbystore();
                    }

                    $scope.getselectedproductforAllstores =function (itemNumber) {
                        $scope.getproductforRT = function () {

                          var data = {
                            "aggTimeUnit": "1d",
                            "startTime": $scope.SalesDataReportstartDate,
                            "endTime": $scope.Reportenddate,
                            "filters": {
                              "item.upc": [itemNumber]
                            }
                          }

                          $scope.showproducts=false;
                          $scope.spin=true;
                          productService.GetSalesPerformance(data).then(function (response) {
                            $scope.ProductsbyRT = response.data.data;
                            $scope.singleProductchartId = productService.generateguid();
                            $scope.rpIndextotal = parseFloat(response.data.total);
                            $scope.totalunitsold=response.data.totalSoldQty;

                       var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }

                            if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                              $scope.spIndex = 0;
                            }
                            else {
                              $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                              $scope.spIndex = $scope.spIndex.toFixed(2);
                            }

                            $scope.barchartData = [];

                            var barchartDataObject = {
                              "name": itemNumber,
                              "income": $scope.rpIndextotal,
                              "expenses": $scope.spIndextotal,
                              "color": "#4C98CF",
                              "color1": "#7F2891",
                              "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                              "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                              "reporttime":$scope.totalreporttime,
                              "comapretime":$scope.totalcomparetime
                     
                            }
                            $scope.barchartData.push(barchartDataObject);
                            var barchartObject = {

                              "id": $scope.singleProductchartId,
                              "data": $scope.barchartData,
                              "itemNumber": itemNumber,
                              "index": $scope.spIndex,
                              "total": $scope.rpIndextotal,
                               "unitsold":unitsold,
                               "arrowdircetion":"glyphicon-arrow-down",
                               "barchart":false
                            }

                            $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                            $scope.singleProductLinechartId = productService.generateguid();
                            $scope.linechartData = [];

                          var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
              var days = b.diff(a, 'days');

              $scope.j=1;

            for (var i = 0; i < $scope.ProductsbyRT.length; i++) {
                        if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=$scope.date;
             }
             else{
               
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=$scope.nextDate;
               $scope.j++;

             }

              if ($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]) {
                 var linechartdataobject = {
                    "date": $scope.ResultDate,
                    "lineColor": "rgb(66, 141, 201)",
                    "value": $scope.ProductsbyRT[i].amt,
                    "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": itemNumber,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)

                       }
                       $scope.linechartData.push(linechartdataobject);
                     }
                  else if($scope.ProductsbyRT[i]){
                       var linechartdataobject = {
                          "date": $scope.ResultDate,
                           "lineColor": "rgb(66, 141, 201)",
                          "value": $scope.ProductsbyRT[i].amt,
                          "value2":0,
                         "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                          "amt1":0.00,
                           "name": itemNumber,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                        }
                     $scope.linechartData.push(linechartdataobject);
                    }
                    else if($scope.ProductsbyCT[i]){
                       var linechartdataobject = {
                          "date": $scope.ResultDate,
                          "lineColor": "rgb(66, 141, 201)",
                          "value": 0,
                          "value2": $scope.ProductsbyCT[i].amt,
                          "amt": 0.00,
                          "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                          "name": itemNumber,
                          "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                        }
                     $scope.linechartData.push(linechartdataobject);
                    }
                     }

                          var linechartObject={
                            "id":$scope.singleProductLinechartId,
                            "data":$scope.linechartData,
                            "itemNumber":itemNumber,
                            "index":$scope.spIndex,
                            "total":$scope.rpIndextotal,
                              "unitsold":unitsold,
                               "arrowdircetion":"glyphicon-arrow-down",
                               "linechart":true
                          }
                            $scope.selectedItemsDataAraayforlinechart.push(linechartObject);

                               var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }
                          $scope.itemsList.push(objectitems);
                           $scope.showproducts=true;
                           $scope.spin=false;

                            }, function (response) {
                              console.log(response);
                            }
                          );

                        }

                        var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters": {
                            "item.upc": [itemNumber]
                          }
                        }

                        productService.GetSalesPerformance(data1).then(function (response) {
                            $scope.ProductsbyCT = response.data.data;
                            if(response.data){
                              $scope.unitsoldbycp=response.data.totalSoldQty
                            }
                          $scope.spIndextotal = parseFloat(response.data.total);
                            $scope.getproductforRT();
                          }, function (response) {
                            console.log(response);
                          }
                        );
                    }



      $scope.imgbar = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAIAAAACAAQMAAAD58POIAAAABlBMVEUAAAAAAAClZ7nPAAAAAXRSTlMAQObYZgAAACtJREFUSMdj+A8GDAgwKjAqwDAKcIH/cDAqMCqAKTAKcOUXaDCNCowKIAAAP/H8Lm4fPi4AAAAASUVORK5CYII=";
      $scope.imgline = "data:image/png;base64,";
      $scope.imgmap = "data:image/png;base64,";


      $scope.imagebuffer = "data:image/jpeg;base64,";


      $scope.spinneractive = false;

      $rootScope.$on('us-spinner:spin', function (event, key) {
        $scope.spinneractive = true;
      });

      $rootScope.$on('us-spinner:stop', function (event, key) {
        $scope.spinneractive = false;
      });


      /**************************END********************************/


      /* ******   start screen individual product  ******** */

      $scope.ListdeptProduct = [];

      $scope.selectdepartmentProducts = function () {

        $scope.listproducts = function () {

          var DataName = {
            "aggTimeUnit": "1d",
            "startTime": $scope.SalesDataReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel": "L",
            "bucketType": "deptName"
          }


          productService.GetSalesPerformance(DataName).then(function (response) {
              var count = 0;
              $scope.total =response.data.total;

             if($scope.total){
             $filter('number')($scope.total, $scope.total.length)

             }

               if(response.data.data){
                 for (var i = 0; i < response.data.data.length; i++) {
                // if(i<10) {
                var object = {
                  "amount": response.data.data[i].amt, "Name": response.data.data[i].id,
                  "id": $scope.listdeptproductname[i].id
                };
                $scope.ListdeptProduct.push(object);
              }
               }

               $scope.selectedvalue1=$scope.ListdeptProduct[0];
                $scope.selectdropdown();
              $scope.showpiechart = true;
              //  $scope.SalesPerformance=true;
            }, function (response) {

            }
          );
        }

        var DataId = {
          "aggTimeUnit": "1d",
          "startTime": $scope.SalesDataReportstartDate,
          "endTime": $scope.Reportenddate,
          "bucketLevel": "L",
          "bucketType": "deptId"
        }

        productService.GetSalesPerformance(DataId).then(function (response) {
            var count = 0;
            $scope.total = response.data.total;
             if($scope.total){
              $filter('number')($scope.total, $scope.total.length)
             }
            $scope.ListdeptProductid = [];
            $scope.listdeptproductname = response.data.data;
            $scope.listproducts();
            $scope.showpiechart = true;
          }, function (response) {

          }
        );
      }


      $scope.selectdepartmentProductsbycpg = function () {

        $scope.listproductsforcpg = function () {

          var DataName = {
            "aggTimeUnit": "1d",
            "startTime": $scope.SalesDataReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel": "L",
            // "bucketType": "deptName",
             "filters": {
              "items.mfgId" : [$scope.mfgId],
            }
          }


          productService.GetSalesPerformanceByAllRetailerswithoutsize(DataName).then(function (response) {
              var count = 0;
              $scope.total =response.data.total;

            $filter('number')($scope.total, $scope.total.length)

              console.log("selectdepartmentProductsbycpg",response);
              for (var i = 0; i < response.data.data.length; i++) {
                // if(i<10) {
                var object = {
                  "amount": response.data.data[i].amt, 
                  "Name": response.data.data[i].id,
                  "id": $scope.listdeptproductname[i].id
                };

                $scope.ListdeptProduct.push(object);
              }
              //console.log("categories...",$scope.ListdeptProduct);

            }, function (response) {

            }
          );

        }

        var DataId = {
          "aggTimeUnit": "1d",
          "startTime": $scope.SalesDataReportstartDate,
          "endTime": $scope.Reportenddate,
          "bucketLevel": "L",
          // "bucketType": "deptId",
            "filters": {
              "items.mfgId" : [$scope.mfgId],
            }
        }

        productService.GetSalesPerformanceByAllRetailerswithoutsize(DataId).then(function (response) {
            var count = 0;
            $scope.total = response.data.total;

          $filter('number')($scope.total, $scope.total.length)
            $scope.ListdeptProductid = [];
            $scope.listdeptproductname = response.data.data;
            $scope.listproductsforcpg();
          }, function (response) {
          }
        );
      }


      $scope.ctrl = {
              'treeData':[],
              'treeDropdownstatus':false,
              'selected': {
                "id": 1,
                "name": "All Retailers",
                "retailerId":"All Retailers"

              }
            };

            $scope.ctrl.treeData = [
            {
              "id": 1,
              "name": "All Retailers",
              "retailerId":"All Retailers"
            }
            ]

             $scope.ctrl.treeData1 = [
            {
              "id": 1,
              "name": "All Retailers",
              "retailerId":"All Retailers"

            }
            ]


            $rootScope.selected={"id": 1,
            "name": "All Retailers",
            "retailerId":"All Retailers"};

             $scope.savedvalue=dashBoardService.getsavestoreselected();

            //console.log("already saved value....",$scope.savedvalue);

            if($scope.savedvalue==null){
                 $scope.defaultstoreselected=$rootScope.selected;
            dashBoardService.setsavestoreselected($scope.defaultstoreselected);
            }

            $scope.retailerList=[];
            $scope.collection1=[{
              "id":1,
              "name":"All Retailers",
              "retailername":"All Retailers",
              "retailerId":"All Retailers"
            }]
            $scope.storesByRetailer=[{
              "normalId":1,
              "id":1,
              "name":"All Retailers",
              "retailername":"All Retailers",
              "retailerId":"All Retailers"
            }]


            
            $scope.ctrl.treeDropdownstatus=false;
             $scope.dmaStoreList=[];
         $scope.getDmaList=function(value){
         sessionStorage.DMAsuggest = value;
             return dashBoardService.getDMAforCPG().then(function(response){
               $scope.dmaList=[];
                //console.log("dmaListforCPG....",response);

                // var dmaobject={
                //   "id": 2,
                //   "name": "DMA",
                //   "DmaName":"DMA"
                // }
                // $scope.id=$scope.id+1;
                //  $scope.dmaList1=response.data;
               //$scope.ctrl.treeData.push(dmaobject);
               //$scope.dmaList=response.data;
              for(var i=0;i<response.data.length;i++){
              var dma={
                    //"id":$scope.id,
                    "name":response.data[i].dma_name,
                    "dmaId":response.data[i].dma_id,
                    "stores":response.data[i].stores
              }
               // $scope.ctrl.treeData.push(dma);
               $scope.dmaList.push(dma);
                // $scope.id=$scope.id+1;
             }

              console.log("dmaListforCPG....",$scope.dmaList);
               return $scope.dmaList;
             //$scope.dma=$scope.dmaList[0].name;
                
             // }
             //    var retailerobject={
             //      "id": $scope.id,
             //      "name": "RETAILER",
             //      "DmaName":"RETAILER"
             //    }
             //   // $scope.ctrl.treeData.push(retailerobject);
                // $scope.id=$scope.id+1;

                //  $scope.arrarIdlength=$scope.dmaList1.length+3;

               // $scope.getAllRetailers();
              },function(error){
              });
             }

$scope.dma="";
          $scope.selectDMA=function(value){
            $scope.productList=[];
              $scope.ctrl.selected = {              
                "id": 1,
                "name": "All Retailers",
                "retailerId":"All Retailers"
              
            };
              dashBoardService.setsavestoreselected($scope.ctrl.selected);
              $scope.dmaStoreList=[];
              console.log("value......",value);
              for(var i=0; i< $scope.dmaList.length; i++){
                if(value == $scope.dmaList[i].name){
                for(var j=0; j< $scope.dmaList[i].stores.length;j++){
                    $scope.dmaStoreList.push($scope.dmaList[i].stores[j].toString());
                     sessionStorage.DMAID=$scope.dmaList[i].dmaId;
                     dashBoardService.saveselectedDMA($scope.dmaList[i]);
                   }
                }
              }
         
              $scope.dma=value;             
             
               console.log("ctrl.selected",$scope.ctrl.selected);
               $scope.retailerId=$scope.ctrl.selected.retailerId;
                var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",               
                "filters": {
                //"items.mfgId":[$scope.mfgId],
                "storeId": $scope.dmaStoreList,
                "item.categoryDesc": [$scope.categoryalreadyselected]
              }
            }    
          productService.getitemsbycategory(deptdataRT).then(function (response) {
              console.log(response.data);           
              for (var i = 0; i < response.data.data.length; i++) {
                 if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }           
             }
          });
       // console.log("$scope.productList", $scope.productList);
     var dataobject={
               "method":"dma",
               "compareStartDate":$scope.ComparestartDate,
               "compareEndDate":$scope.Compareenddate,
               "reportEndDate":$scope.Reportenddate,
               "reportStartDate":$scope.ReportstartDate,
               "storeid":$scope.dmaStoreList,
               "itemNumber":$scope.deptitemNameArray,
               "salesDatastartdate":$scope.SalesDataReportstartDate,
               "salesDataenddate":$scope.SalesDataComparestartDate
               }


    if($scope.selectedItemsDataAraayforbarchart.length>0){
      for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }

            if($scope.selectedgraph =='line'){
               $rootScope.$emit('LinechartEvent', dataobject);
               $scope.linechartapicallstatus=true;
               $scope.barchartapicallstatus=false;
                var productclicked= $rootScope.$on('chartdata', function (event, data) {
                  $scope.itemsList[0].linechart.data= data
                  // console.log(" $scope.itemsList", $scope.itemsList);
               });
            }
            else{
               $rootScope.$emit('myCustomEvent', dataobject);
               $scope.barchartapicallstatus=true;
               $scope.linechartapicallstatus=false;
            }
          }
     }

            








            $scope.id=2;
            $scope.makeStoresList=function(retailerid,retailerName,status){
              dashBoardService.storeListforCPG().then(function (response) {

                $scope.storesList=response.data;
             for(var i=0; i<$scope.getStoresData.length; i++){
                var storesssobject={
                 "id":   $scope.id,
                 "name": $scope.getStoresData[i].retailername,
                 "retailerId":$scope.getStoresData[i].retailerid,
                 "children": [],
               }

               for(var j=0;j<$scope.storesList.length;j++){
                 if($scope.storesList[j].retailer_id == $scope.getStoresData[i].retailerid){
                 var id=$scope.storesByRetailer.length+1;
                 $scope.id=$scope.id+1;
                 var object={
                   "id":$scope.id ,
                   "name":$scope.storesList[j].store_name,
                   "storeid":$scope.storesList[j].store_id,
                   "retailerId":$scope.getStoresData[i].retailerid
                 }
                 $scope.storesByRetailer.push(object);
                 storesssobject.children.push(object);

               }}

               $scope.id=$scope.id+1;
               $scope.storesByRetailer.push(object);

               $scope.selectedOption = $scope.storesByRetailer[0];
               $scope.ctrl.treeData1.push(storesssobject);
              // $scope.ctrl.treeDropdownstatus=false;

              var index=$scope.retailerData.length-1;

                if($scope.retailerData[index].retailer_name==$scope.getStoresData[i].retailername){

                  $timeout(function(){

                  $scope.arrayid=1;
                  for(var i=0;i<$scope.retailerData.length;i++){
                    var findretailer=$filter('filter')($scope.ctrl.treeData1, 
                    {name : $scope.retailerData[i].retailer_name}, true);

                    if(findretailer.length>0){
                       $scope.arrayid=$scope.arrayid+1;
                    findretailer[0].id=$scope.arrayid;
                    for(var j=0;j<findretailer[0].children.length;j++){

                         $scope.arrayid=$scope.arrayid+1;
                      findretailer[0].children[j].id=$scope.arrayid;
                    }
                       $scope.ctrl.treeData.push(findretailer[0]);
                    }
                  }
                $scope.ctrl.treeDropdownstatus=true;
                },1400)

                }
               }
             }, function (response) {
              console.log(response);
            }
            );
            }
             $scope.allapicalled=false;
             $scope.getStoresData=[];
            $scope.getStoresByRetailer=function(){
            for (var i = 0; i < $scope.retailerData.length;i++) {
              $scope.retailerList.push($scope.retailerData[i]);
              sessionStorage.retailer=$scope.retailerData[i].retailer_id;
            if(i==$scope.retailerData.length-1){
              $scope.allapicalled=true;
            }
            else{
              $scope.allapicalled=false;
            }
            //$scope.makeStoresList($scope.retailerData[i].retailer_id,$scope.retailerData[i].retailer_name,$scope.allapicalled);
              $scope.getStoresData.push({
                   "retailerid":$scope.retailerData[i].retailer_id,
                   "retailername":$scope.retailerData[i].retailer_name
                 });
          }
          $timeout(function() {
                    $scope.makeStoresList();                   
                  }, 1000);
              }
            

            $scope.rids;
            $scope.RetailerIds=[];

    $scope.getAllRetailers=function(){
    dashBoardService.getRetailerforCPG().then(function (response) {
    $scope.retailerData=response.data;
    for(var i=0;i<$scope.retailerData.length;i++){
    $scope.RetailerIds.push($scope.retailerData[i].retailer_id.toString());
    if($scope.rids){
       $scope.rids=$scope.rids+","+$scope.retailerData[i].retailer_id;
    }
    else{
       $scope.rids=$scope.retailerData[i].retailer_id;
    }   
    }
    productService.saveallRetailers($scope.RetailerIds);
    sessionStorage.rids=$scope.rids;
    $scope.getcategoriesList();
    $scope.getStoresByRetailer();
    }, function (response) {
      console.log(response);

    }
    );
    }


      /* *************************************** End  ******************************************************/

        /******* distributor api calls ********/
        
        $scope.BrandsList=[
           {
            "brand_id":"",
            "brand_name":"ALL BRANDS"
           }];

           $scope.BrandIdsList=[];
           $scope.brandId;

        $scope.getDistributorBrands=function(){
        dashBoardService.getDistributorBrands().then(function(response){
        
        for(var i=0;i<response.data.length;i++){
        $scope.BrandsList.push(response.data[i]);
        $scope.BrandIdsList.push(response.data[i].brand_id);
        if($scope.brandId){
          $scope.brandId=$scope.brandId+","+response.data[i].brand_id;
        }
        else{
          $scope.brandId=response.data[i].brand_id;
        }
        }
        sessionStorage.brandIdsList=$scope.brandId;
        for(var i=0;i<$scope.BrandsList.length;i++){
        $scope.BrandsList[i].brand_name=$scope.BrandsList[i].brand_name.toUpperCase();
        }
         $scope.selectedBrand=$scope.BrandsList[0];
         $scope.selectedBrandId=$scope.BrandsList[0].brand_id;
         dashBoardService.setBrandidList($scope.BrandIdsList);
         
        // $scope.getcategoriesforAllBrands();

         $scope.savedBrand=dashBoardService.getselectedBrand();
        if($scope.savedBrand!=null){
        if($scope.savedBrand.brand_name!="ALL BRANDS"){
        for(var i=0;i<$scope.BrandsList.length;i++){
        if($scope.savedBrand.brand_id==$scope.BrandsList[i].brand_id){
         $scope.selectedBrand=$scope.BrandsList[i];
         $scope.selectedBrandId=$scope.savedBrand.brand_id;
        }
        }
        }
        }
        else{
            dashBoardService.saveselectedBrand($scope.selectedBrand);
        }
        },function(response){
           console.log("error msg..",response);
        });
        }

           
        $scope.getcategoriesforAllBrands=function(){
        productService.getallCategoriesbyBrands().then(function(response){
       // $scope.categoriesList=undefined;
       $scope.categoriesList=[];
        //$scope.categoriesList=response.data;
        for(var i=0;i<response.data.length;i++){
          $scope.categoriesList.push(response.data[i]);
        }
        //console.log("categories List....",$scope.categoriesList);
        $scope.selectedcategory=$scope.categoriesList[0];
        $scope.selectdropdown();
        },function(response){
          console.log(response);
        });
      }
        
        $scope.getCategoriesforSingleBrand=function(brand){
           var reporttimeRequest= {
            "aggTimeUnit": "1d",
            "startTime": $scope.SalesDataReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "filters": {
              "items.brandId" :[brand]
            },
            "bucketType" : "categoryDesc"
           }
        dashBoardService.gettopcategoriesbyBrands(reporttimeRequest).then(function (response) {
          $scope.categoriesList=[];
         // console.log("response...",response.data);
        //$scope.categoriesList=response.data;
        for(var i=0;i<response.data.data.length;i++){
          var categoryobject={
            "category_description":response.data.data[i].id
          }
          $scope.categoriesList.push(categoryobject);
        }
          // console.log("categories Response....",$scope.categoriesList);
           $scope.selectedcategory=$scope.categoriesList[0];
           $scope.selectdropdown();
        }, function (response) {
             console.log(response);
        }
        );
        }

        $scope.brandChange=function(brand){
          $scope.selectedBrand=brand;
          $scope.selectedBrandId=brand.brand_id;
          $scope.selectedItemsDataAraayforbarchart=[];
          $scope.selectedItemsDataAraayforlinechart=[];
          $scope.itemsList=[];
         dashBoardService.saveselectedBrand($scope.selectedBrand);
          if($scope.selectedBrand.brand_name=="ALL BRANDS"){
            $scope.getcategoriesforAllBrands();
          }
          else{
           $scope.getCategoriesforSingleBrand($scope.selectedBrandId);
          }
        }

     $scope.checkbeforeinsert = function(productobject){
        var flag =  true;
       $scope.productList.forEach(function (item) {
          if(item.itemName === productobject.itemName){
            flag = false;
          }
      });
       return flag;
    }

       
       $scope.deptmentid;
      $scope.subproductListbyDistributor= function (deptid,brandList) {
      sessionStorage.itemssize=500;
        $scope.deptmentid = deptid;
          $scope.getdata;
        var deptdataRT ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
            "item.categoryDesc" : [deptid]
             // "items.brandId" :brandList
            }
          }

            var deptdataRT1 ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc"
            // "filters": {
            //     "item.categoryDesc" : [deptid]
            //      // "items.brandId" :brandList
            // }
          }
         if(deptid!=""&&deptid!=undefined){
           $scope.getdata = deptdataRT;
         }else{
           $scope.getdata = deptdataRT1;
         }

          productService.getitemsbycategory($scope.getdata).then(function (response) {
              $scope.subproductlists = [];
              $scope.productList = [];
              for (var i =0;i<response.data.data.length;i++) {

          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
                  var object = {
                    "itemName": response.data.data[i].upc,
                    "name": $scope.productName,
                    "deptId": response.data.data[i].deptId,
                    "amt": response.data.data[i].amt,
                    "cp": response.data.data[i].cp,
                    "namecp": response.data.data[i].name,
                    "category":deptid
                  };
                  var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus",
                    "category":deptid
                  }
                  // $scope.productList.push(productobject);
                  $scope.subproductlists.push(object);
                   if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }
              }
              if($scope.individualcategoryproduct==true){
               $scope.optionpname=window.localStorage['pname'];
               $scope.option=window.localStorage['selectoption1'];
               $scope.optionid=window.localStorage['iddept'];
               $scope.dataplusmark=window.localStorage['dataplus'];
               $scope.optionpitemNumber=window.localStorage['itemNum'];
                 var data ={
                  "deptId":$scope.optionid,
                  "itemName":$scope.optionpitemNumber,
                  "productName":$scope.optionpname,
                  "status":"plus"
                 }
                 if($scope.getcompproduct == null){
                     $scope.selectedProduct(data);
                 }else if($scope.getcompproduct.length == 2){
                  for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else if($scope.getcompproduct.length == 2){
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                }
                   for(var i=0; i< $scope.getcompproduct.length; i++){
                  $scope.selectedProduct($scope.getcompproduct[i]);
                }
               }
              }
              else{
                if($scope.getcompproduct == null){
                   $scope.selectedProduct($scope.productList[0]);
                 }else{
                  for(var i=0;i<$scope.getcompproduct.length;i++){
                 var results=$filter('filter')($scope.productList,{itemName:$scope.getcompproduct[i].itemName}, true);
                 if(results.length>0){
                   $scope.getcompproduct[i]=results[0];
                 }
                 else{
                  $scope.productList.push($scope.getcompproduct[i]);
                 }
                }
                   for(var i=0; i< $scope.getcompproduct.length; i++){
                  $scope.selectedProduct($scope.getcompproduct[i]);
                }
               }
              }
            }, function (response) {
              console.log(response);
            }
          );
           
      }


      $scope.distributeravgBasket=[];

  
   $scope.avgBasketforDistributorByRT = function (itemNumber, avgBasketTotalByCP) {
    var ShoppingTripsdataRT = {
      "aggTimeUnit": "1d",
           "startTime":$scope.ReportstartDate,
            "endTime":$scope.Reportenddate,
            "rid" : "000",
           "filters": {
            "item.upc" : [itemNumber]
        }
       }
       productService.getAvgBasketforDistributor(ShoppingTripsdataRT).then(function (response) {
          $scope.avgBasketTotal = response.data.stats.avg;
          $scope.avgBasketTotalByRT = parseFloat(response.data.stats.avg);
          if (avgBasketTotalByCP == 0 || avgBasketTotalByCP == null || isNaN(avgBasketTotalByCP)) {
            $scope.ABtotal = 0;
            $scope.distributeravgBasket.push({UPC:itemNumber,ABtotal:$scope.ABtotal});
          }
          else {
            $scope.ABtotal = $scope.avgBasketTotalByRT / avgBasketTotalByCP;
            $scope.ABtotal = $scope.ABtotal.toFixed(2);
            $scope.distributeravgBasket.push({UPC:itemNumber,ABtotal:$scope.ABtotal});
          }
           for(var i=0;i<$scope.itemsList.length;i++){
             for(var j=0; j<$scope.distributeravgBasket.length; j++){
               if($scope.itemsList[i].itemNumber==$scope.distributeravgBasket[j].UPC){
                 $scope.itemsList[i].linechart.avgbasket=$scope.distributeravgBasket[j].ABtotal;
                 $scope.itemsList[i].barchart.avgbasket=$scope.distributeravgBasket[j].ABtotal;
             }
          }
         }
      }, function (response) {
        console.log(response);
      }
      );
   }




      $scope.avgBasketforDistributor = function (upc) {   
         var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "rid" : "000",
        "filters": {
        "item.upc" : [upc]
        }
        }
        productService.getAvgBasketforDistributor(ShoppingTripsdata).then(function (response) {
          if(response.data.stats){
          $scope.avgBasketTotalByCP = parseFloat(response.data.stats.avg);
   
             $scope.avgBasketforDistributorByRT(upc, $scope.avgBasketTotalByCP);
         } 
        }, function (response) {
          console.log(response);
        }
        );
        }
        
    

       $scope.distributershoppingTrip=[];

       $scope.ShoppingTripsforDistributorbyRT = function (itemNumber,ShoppingTripsCPindex) {
        var ShoppingTripsdataRT = {
          "aggTimeUnit": "1d",
         "startTime":$scope.ReportstartDate,
          "endTime":$scope.Reportenddate,
          "rid" : "000",
          "filters": {
           "terms" : {
            "item.upc" : [itemNumber]
          }
        }
       }

       productService.getShoppingTripsforDistributor(ShoppingTripsdataRT).then(function (response) {
            $scope.ShoppingTripsTotalbyRT = response.data.total;
            $scope.ShoppingTripsRTindex = parseFloat(response.data.total);

            if(ShoppingTripsCPindex == 0 || ShoppingTripsCPindex == null || isNaN(ShoppingTripsCPindex)) {
              $scope.STIndex = 0;
              $scope.distributershoppingTrip.push({UPC:itemNumber,STIndex:$scope.STIndex});
            }
            else{
              $scope.STIndex = $scope.ShoppingTripsRTindex / ShoppingTripsCPindex;
              $scope.STIndex = $scope.STIndex.toFixed(2);
               $scope.distributershoppingTrip.push({UPC:itemNumber,STIndex:$scope.STIndex});
            }
          for(var i=0;i<$scope.itemsList.length;i++){
            for(var j=0; j<$scope.distributershoppingTrip.length; j++){
                if($scope.itemsList[i].itemNumber == $scope.distributershoppingTrip[j].UPC){
                   $scope.itemsList[i].linechart.shoppingtrips=$scope.distributershoppingTrip[j].STIndex;
                  $scope.itemsList[i].barchart.shoppingtrips=$scope.distributershoppingTrip[j].STIndex;
                }
             }
           }
            }, function (response) {
              console.log(response);
            }
            );
       }


  $scope.ShoppingTripsforDistributor = function (upc) {     
   var ShoppingTripsdata = {
        "aggTimeUnit": "1d",
        "startTime": $scope.ComparestartDate,
        "endTime": $scope.Compareenddate,
        "rid" : "000",
        "filters": {
            "terms" : {
            "item.upc" : [upc]
          }
        }
        }
        productService.getShoppingTripsforDistributor(ShoppingTripsdata).then(function (response) {
          $scope.ShoppingTripsCPindex = parseFloat(response.data.total);
          $scope.ShoppingTripsforDistributorbyRT(upc, $scope.ShoppingTripsCPindex);
        }, function (response) {
          console.log(response);
        }
        );
    }


    $scope.getproductBySelectedItemforDistributor=function(deptmName,productname){
    $scope.getproductforRTforDistributor=function () {

          var data1 = {
              "aggTimeUnit": "1d",
              "startTime": $scope.SalesDataComparestartDate,
              "endTime": $scope.Compareenddate,
              "filters": {
              "item.upc": [deptmName]
              }
              }

          productService.GetSalesPerformanceByAllRetailerswithoutsize(data1).then(function (response) {
              $scope.ProductsbyCT = response.data.data;
              if(response.data){
              $scope.unitsoldbycp=response.data.totalSoldQty
              }
          $scope.spIndextotal = parseFloat(response.data.total);
          var data ={
                     "aggTimeUnit": "1d",
                     "startTime":$scope.SalesDataReportstartDate,
                     "endTime":$scope.Reportenddate,
                     "filters":{
                     "item.upc":[deptmName]
                     }
                    }

                    $scope.showproducts=false;
                    $scope.spin=true;
              productService.GetSalesPerformanceByAllRetailerswithoutsize(data).then(function (response) {
                    $scope.ProductsbyRT = response.data.data;
                    $scope.singleProductchartId = productService.generateguid();
                    $scope.rpIndextotal = parseFloat(response.data.total);
                    $scope.totalunitsold=response.data.totalSoldQty;
                    var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }
                    if($scope.spIndextotal==0||$scope.spIndextotal==null){
                        $scope.spIndex = 0;
                    }
                    else{
                        $scope.spIndex=$scope.rpIndextotal/$scope.spIndextotal;
                        $scope.spIndex=$scope.spIndex.toFixed(2);
                    }
                    $scope.barchartData = [];
                    var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                      }
                    $scope.barchartData.push(barchartDataObject);
                      var barchartObject = {
                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                    }
                    $scope.selectedItemsDataAraayforbarchart.push(barchartObject);
                      /*$scope.shareOfCategoryByAllRetailers(deptmName);
                      $scope.ShareOfBasketByAllRetailer(deptmName);*/

                        $scope.avgBasketforDistributor(deptmName);
                        $scope.ShoppingTripsforDistributor(deptmName)
                        //$scope.ShoppingTripsFunction(deptmName);

                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];
                      var a = moment($scope.ReportstartDate);
                      var b = moment($scope.Reportenddate);
                      var days = b.diff(a, 'days');
                      $scope.j=1;

             for (var i=0;i<=days;i++){
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
               $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");;
               $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");;
               $scope.j++;
             }
              //here
              if($scope.ProductsbyCT&&$scope.ProductsbyRT){
              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){
                var linechartdataobject = {
                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                   "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                   "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                   "name": deptmName,
                   "productName": productname,
                   "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                   "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                    }
                    $scope.linechartData.push(linechartdataobject);
                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                        "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                        "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                        "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                        "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                        "unitSoldbyRp":0,
                        "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }

                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                        "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){
                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                        "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                        "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                        "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                  }
                  }
                  }
                var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                              "avgbasket":"",
                              "unitsold":unitsold,
                              "arrowdircetion":"glyphicon-arrow-down",
                              "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);
                            var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }
                             $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                             $scope.$applyAsync();

                            }, function (response) {
                              console.log(response);
                            }
                          );
                          }, function (response) {
                            console.log(response);
                          }
                        );
                      }
                         $scope.getproductforRTforDistributor();
                      }
        
        /******** end of distributor api calls *******/

      //dateRangePicker for CompareTimePeriod

      $scope.comparestart=  dashBoardService.getcomparestartdate();

if($scope.comparestart==undefined){

  /*var Reportenddate = moment("2016-11-30");
 var ReportstartDate = moment("2016-11-01");

    var compareEnd=moment(Reportenddate).subtract(1,'year');
    var compareStart=moment(ReportstartDate).subtract(1,'year');

*/

var ReportstartDate= moment().subtract(7, 'days');
    var Reportenddate=moment().subtract(1, 'days');

  var  compareEnd=moment(ReportstartDate).subtract(1,'days');
  var  compareStart=moment(ReportstartDate).subtract(7,'days');


   //var compareEnd = moment("2016-08-31");
  // var compareStart  = moment("2016-08-01");


}
else{
  var compareStart= dashBoardService.getcomparestartdate();
    var compareEnd=dashBoardService.getcompareenddate();
  var ReportstartDate=  dashBoardService.getreportstartdate();
    var Reportenddate=dashBoardService.getreportenddate();

}

      //var end = moment().subtract(17, 'days');
      //var start = moment().subtract(19, 'days');

      var end = moment(compareEnd);
      var start = moment(compareStart);

      $scope.data={
       selectedStoreId:''
      }

      function cmpare(start, end) {

        startDate = start;
        endDate = end;

        compareStart=start;
        compareEnd=end;

    dashBoardService.setcomparestartdate(start);
    dashBoardService.setcompareenddate(end);
    dashBoardService.setreportstartdate(ReportstartDate);
    dashBoardService.setreportenddate(Reportenddate);

    dashBoardService.setdashboardcacheStaus(false);
    dashBoardService.setproductscacheStatus(false);
    dashBoardService.setsalesregioncacheStatus(false);

        $scope.ComparestartDate = startDate.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.Compareenddate = endDate.format('YYYYMMDD') + 'T235959.000-0000';
        $scope.SalesDataComparestartDate =startDate.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.totalcomparetime=startDate.format('MMM Do YYYY')+"-"+endDate.format('MMM Do YYYY');
        $scope.compareTimePeriod = startDate.format('MMM DD YYYY') + " - " + endDate.format('MMM DD YYYY');
         if($scope.role=="retailer"){
        if ($scope.selectedStoreId == 'allStores'||$scope.selectedStoreId==undefined)
         {
                           var dataobject={
                              "method":"allstores",
                             "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
                                if($scope.selectedItemsDataAraayforbarchart.length>0){
                                     if($scope.selectedgraph =='line'){
                            $rootScope.$emit('LinechartEvent', dataobject);
                            $scope.linechartapicallstatus=true;
                            $scope.barchartapicallstatus=false;
                              }
                          else{
                            $rootScope.$emit('myCustomEvent', dataobject);
                            $scope.barchartapicallstatus=true;
                            $scope.linechartapicallstatus=false;
                        }
                      }
         }
         else{
           var dataobject={
             "method":"store",
             "compareStartDate":  $scope.ComparestartDate,
             "compareEndDate":$scope.Compareenddate,
             "reportEndDate":$scope.Reportenddate,
             "reportStartDate":$scope.ReportstartDate,
             "storeid":$scope.data.selectedStoreId,
             "itemNumber":$scope.deptitemNameArray,
             "salesDatastartdate":$scope.SalesDataReportstartDate,
             "salesDataenddate":$scope.SalesDataComparestartDate
          }
        if($scope.selectedItemsDataAraayforbarchart.length>0){
        if($scope.selectedgraph =='line'){
       $rootScope.$emit('LinechartEvent', dataobject);
       $scope.linechartapicallstatus=true;
       $scope.barchartapicallstatus=false;
         }
     else{
       $rootScope.$emit('myCustomEvent', dataobject);
       $scope.barchartapicallstatus=true;
       $scope.linechartapicallstatus=false;
      }
      }
      }
      }
      else if($scope.role=="distributor"){
          var dataobject={
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }
      if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
       }
         }
      else if($scope.role=="cpg"){
      $scope.selecteRecord= dashBoardService.getselectedrecordfromdropdown();
      if($scope.selecteRecord){
      if($scope.selecteRecord.hasOwnProperty("dmaId")){
      $scope.dmaStoreList=[];
      for(var i=0;i<$scope.selecteRecord.stores.length;i++){
      $scope.dmaStoreList.push($scope.selecteRecord.stores[i].toString());
       }
       var dataobject={
              "method":"dma",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.dmaStoreList,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }
      if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
       }
     }
     else{
    if($scope.selecteRecord.name=="All Retailers"){
        var dataobject={
              "method":"allretailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
      }
      if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
       }
      }
     else{
      if($scope.selecteRecord.hasOwnProperty("children")){
      sessionStorage.user=$scope.selecteRecord.retailerId;
      $scope.retailerid=$scope.selecteRecord.retailerId;
      var dataobject={
              "method":"retailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }
      if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
        }
        }
      else{
          //console.log('coming to here after selecting');
          sessionStorage.user=$scope.selecteRecord.retailerId;
          $scope.storeId = $scope.selecteRecord.storeid;
          sessionStorage.storeId = $scope.storeId;
          var dataobject={
              "method":"store",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.storeId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }

      if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
      }
      }
      }
    }
    }
    else {
      var dataobject={
               "method":"allretailer",
               "compareStartDate":  $scope.ComparestartDate,
               "compareEndDate":$scope.Compareenddate,
               "reportEndDate":$scope.Reportenddate,
               "reportStartDate":$scope.ReportstartDate,
               "storeid":$scope.data.selectedStoreId,
               "itemNumber":$scope.deptitemNameArray,
               "salesDatastartdate":$scope.SalesDataReportstartDate,
               "salesDataenddate":$scope.SalesDataComparestartDate
      }
    if($scope.selectedgraph =='line'){
      $rootScope.$emit('LinechartEvent', dataobject);
      $scope.linechartapicallstatus=true;
      $scope.barchartapicallstatus=false;
        }
    else{
      $rootScope.$emit('myCustomEvent', dataobject);
      $scope.barchartapicallstatus=true;
      $scope.linechartapicallstatus=false;
     }
     }
     }
     }

      function cmpareinit(start, end) {
        startDate = start;
        endDate = end;
        $scope.ComparestartDate = startDate.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.Compareenddate = endDate.format('YYYYMMDD') + 'T235959.000-0000';
        $scope.SalesDataComparestartDate =startDate.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.compareTimePeriod = startDate.format('MMM DD YYYY') + " - " + endDate.format('MMM DD YYYY');
        $scope.totalcomparetime=startDate.format('MMM Do YYYY')+"-"+endDate.format('MMM Do YYYY');
         $scope.comparetime=startDate.format('MM/DD/YYYY')+"-"+endDate.format('MM/DD/YYYY');
        $scope.$applyAsync();
      }

      $scope.setcomparetimeforproduct=function(){
            end = moment(compareEnd);
            start = moment(compareStart);
      $('input[name="daterange"]').daterangepicker(
        {
           maxDate: new Date(),
           "opens":"left",
          locale: {
            format: 'MM/DD/YYYY'
          },
          startDate: start,
          endDate: end
        }
        , cmpare);
    }

      cmpareinit(start, end);
      $scope.setcomparetimeforproduct();
      var startDate;
      var endDate;
      //DateRangePicker for ReportTime Period
      var startDate;
      var endDate;
    //  var end = moment().subtract(14, 'days');
    //  var start = moment().subtract(16, 'days');
    var end = moment(Reportenddate);
    var start = moment(ReportstartDate)
      function daterangepickerCallBackreporttimeindividual(start, end,label) {
        $('#reporttime-individual span').html(start.format('MM/DD/YYYY') + ' - ' + end.format('MM/DD/YYYY'));
        $('#reporttime-individual').data('daterangepicker').setStartDate(start);
        $('#reporttime-individual').data('daterangepicker').setEndDate(end);
    
        startDate = start;
        endDate = end;
        $scope.ReportstartDate = startDate;
        $scope.Reportenddate = endDate;
        ReportstartDate= start;
        Reportenddate=end;
        dashBoardService.settimeperiodlabel(label);
        if(label=="Last week"){
        if($scope.reporttimeranges.indexOf(7)){
          $scope.reporttimeranges.splice(7, 1);
        }
        $scope.selectedtimeperiod=$scope.reporttimeranges[0];
        }
        else if(label=="This Month"){
        if($scope.reporttimeranges.indexOf(7)){
            $scope.reporttimeranges.splice(7, 1);
        }
        $scope.selectedtimeperiod=$scope.reporttimeranges[1];
        }
        else if(label=="Last Month"){
        if($scope.reporttimeranges.indexOf(7)){
          $scope.reporttimeranges.splice(7, 1);
        }
        $scope.selectedtimeperiod=$scope.reporttimeranges[2];
        }
        else if(label=="Quarter 1 (Jan-Mar)"){
        if($scope.reporttimeranges.indexOf(7)){
          $scope.reporttimeranges.splice(7, 1);
        }
        $scope.selectedtimeperiod=$scope.reporttimeranges[3];
        }
        else if(label=="Custom Range"){
        if($scope.reporttimeranges[7]==undefined){
          var obj={"name":"CUSTOM"}
        $scope.reporttimeranges.push(obj);
        }
        $scope.selectedtimeperiod=$scope.reporttimeranges[7];
        }
          if($scope.selectedtimeperiod.name=="LAST 7 DAYS"){
            if($scope.lastsevendays==true){
            $scope.lastsevendays=false;
            compareEnd=moment(start).subtract(1,'days');
            compareStart=moment(start).subtract(7,'days');
            cmpareinit(compareStart,compareEnd);
            $scope.setcomparetimeforproduct();
            }
            else{
          compareEnd=moment(end).subtract(1,'year');
          compareStart=moment(start).subtract(1,'year');
          cmpareinit(compareStart,compareEnd);
          $scope.setcomparetimeforproduct();
            }
          }
          else{
    compareEnd=moment(end).subtract(1,'year');
    compareStart=moment(start).subtract(1,'year');
    cmpareinit(compareStart,compareEnd);
    $scope.setcomparetimeforproduct();
          }
       dashBoardService.setcomparestartdate(compareStart);
       dashBoardService.setcompareenddate(compareEnd);
       dashBoardService.setreportstartdate(start);
       dashBoardService.setreportenddate(end);
       dashBoardService.setdashboardcacheStaus(false);
       dashBoardService.setproductscacheStatus(false);
       dashBoardService.setsalesregioncacheStatus(false);
        $scope.ReportstartDate = start.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.Reportenddate = end.format('YYYYMMDD') + 'T235959.000-0000';
        $scope.SalesDataReportstartDate =start.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.ReportTimePeriodp = start.format('MMM DD YYYY') + ' - ' + end.format('MMM DD YYYY');
        $scope.totalreporttime=start.format('MMM Do YYYY')+"-"+end.format('MMM Do YYYY');
                 // $scope.getUPC=[];
                 // $scope.getprdctName=[];
      if($scope.role=="retailer"){
            //$scope.upcDataArr = [];
            //$scope.avgbasketArr=[];
            //$scope.ShoppingtripArr=[];
            //$scope.itemsList=[];
      if($scope.selectedStoreId=='allStores'||$scope.selectedStoreId==undefined)
                 {
                   $scope.productList = [];
                   var deptdataRT = {
                  "aggTimeUnit": "1d",
                  "startTime": $scope.ReportstartDate,
                  "endTime": $scope.Reportenddate,
                  "bucketLevel" : "L",
                  "bucketType" : "upc",
                  "filters": {
                  "item.deptId": $scope.optionid,
                  "item.categoryDesc": $scope.category 
                }
                }
      productService.RetailerProductbyDepartment(deptdataRT).then(function(response) {
        //console.log(response.data);           
        for (var i=0;i<response.data.data.length;i++) {

           if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
        var productobject = {
            "productName":$scope.productName,
            "itemName":response.data.data[i].upc,
            "deptId":response.data.data[i].deptId,
            "status":"plus"
            }
          if($scope.checkbeforeinsert(productobject)){
            $scope.productList.push(productobject);
          } 
         //$scope.productList.push(productobject);            
         }
         console.log("product list...",$scope.productList);
        var dataobject={
            "method":"allstores",
            "compareStartDate":  $scope.ComparestartDate,
            "compareEndDate":$scope.Compareenddate,
            "reportEndDate":$scope.Reportenddate,
            "reportStartDate":$scope.ReportstartDate,
            "storeid":$scope.data.selectedStoreId,
            "itemNumber":$scope.deptitemNameArray,
            "salesDatastartdate":$scope.SalesDataReportstartDate,
            "salesDataenddate":$scope.SalesDataComparestartDate
            }
          console.log("array...",$scope.selectedItemsDataAraayforbarchart);
          if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
            console.log(response);
          }
          );
          //console.log("$scope.productList", $scope.productList);
          //console.log("getUPC...",$scope.getUPC);
          }
          else{
          $scope.productList = [];
                    var deptdataRT = {
                       "aggTimeUnit": "1d",
                       "startTime": $scope.ReportstartDate,
                       "endTime": $scope.Reportenddate,
                       "bucketLevel" : "L",
                       "bucketType" : "upc",
                       "filters": {
                       "item.deptId": $scope.optionid,
                       "item.categoryDesc": $scope.category 
                       }
                    }     
    productService.RetailerProductbyStore(deptdataRT).then(function (response) {
        console.log(response.data);           
      for (var i =0;i<response.data.data.length;i++){
        if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
          var productobject = {
            "productName":$scope.productName,
            "itemName":response.data.data[i].upc,
            "deptId":response.data.data[i].deptId,
            "status": "plus"
          }
          if($scope.checkbeforeinsert(productobject)){
            $scope.productList.push(productobject);
          }            
          }
                  var dataobject={
                    "method":"store",
                    "compareStartDate":  $scope.ComparestartDate,
                    "compareEndDate":$scope.Compareenddate,
                    "reportEndDate":$scope.Reportenddate,
                    "reportStartDate":$scope.ReportstartDate,
                    "storeid":$scope.data.selectedStoreId,
                    "itemNumber":$scope.deptitemNameArray,
                    "salesDatastartdate":$scope.SalesDataReportstartDate,
                    "salesDataenddate":$scope.SalesDataComparestartDate
                           }
        if($scope.selectedItemsDataAraayforbarchart.length>0){
      for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
        }, function (response) {
            console.log(response);
          }
          );
              console.log("$scope.productList", $scope.productList);
          }
          }
          else if($scope.role=="distributor"){
            $scope.productList=[];
            console.log("..........",$scope.deptmentid);
          var deptdataRT ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
            "item.categoryDesc" : [$scope.deptmentid]
            }
          }
       // console.log("deptdataRT",deptdataRT);
       productService.getitemsbycategory(deptdataRT).then(function (response) {
          $scope.productList = [];
          for (var i =0;i<response.data.data.length;i++) {
             if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
          var productobject = {
              "productName": $scope.productName,
              "itemName": response.data.data[i].upc,
              "deptId": response.data.data[i].deptId,
              "status": "plus",
              "category":$scope.deptmentid
              }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }
          }

           var dataobject={
            "compareStartDate":$scope.ComparestartDate,
            "compareEndDate":$scope.Compareenddate,
            "reportEndDate":$scope.Reportenddate,
            "reportStartDate":$scope.ReportstartDate,
            "storeid":$scope.data.selectedStoreId,
            "itemNumber":$scope.deptitemNameArray,
            "salesDatastartdate":$scope.SalesDataReportstartDate,
            "salesDataenddate":$scope.SalesDataComparestartDate
            }

             if($scope.selectedItemsDataAraayforbarchart.length>0){
      for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
            console.log(response);
          }
          );
      }
      else if($scope.role=="cpg"){
      $scope.selecteRecord=dashBoardService.getselectedrecordfromdropdown();
      if($scope.selecteRecord){
      $scope.productList=[];
      if($scope.selecteRecord.hasOwnProperty("dmaId")){
      $scope.dmaStoreList=[];
      for(var i=0;i<$scope.selecteRecord.stores.length;i++){
      $scope.dmaStoreList.push($scope.selecteRecord.stores[i].toString());
      }
      var deptdataRT ={
          "aggTimeUnit": "1d",
          "startTime": $scope.ReportstartDate,
          "endTime": $scope.Reportenddate,
          "bucketLevel" : "L",
          "bucketType" : "upc",               
          "filters": {
          "storeId": $scope.dmaStoreList,
          "item.categoryDesc": [$scope.categoryalreadyselected]
          }
          }    
          productService.getitemsbycategory(deptdataRT).then(function (response) {
        console.log(response.data);           
        for (var i = 0; i < response.data.data.length; i++) {
          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
        var productobject = {
            "productName": $scope.productName,
            "itemName": response.data.data[i].upc,
            "deptId": response.data.data[i].deptId,
            "status": "plus"
          }
        if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
        }           
        }
        var dataobject={
              "method":"dma",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.dmaStoreList,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }

      if($scope.selectedItemsDataAraayforbarchart.length>0){
      for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
        }, function (response) {
          console.log(response);
        }
       );
     }
     else{
          if($scope.selecteRecord.name=="All Retailers"){
            var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                     // "items.mfgId":[$scope.mfgId],
                     "retailerId" :$scope.RetailerIds,
                     "item.categoryDesc": [$scope.categoryalreadyselected]
                    }
                  }    
          productService.getitemsbycategory(deptdataRT).then(function (response) {
              console.log(response.data);           
          for (var i =0;i<response.data.data.length;i++) {
          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }
             }
             console.log("$scope.productList", $scope.productList);
        var dataobject={
              "method":"allretailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }
          if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
          console.log(response);
        }
       );
         }
         else{
          if($scope.selecteRecord.hasOwnProperty("children")){
           sessionStorage.user=$scope.selecteRecord.retailerId;
           $scope.retailerid=$scope.selecteRecord.retailerId;
           var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                "item.categoryDesc": [$scope.categoryalreadyselected]
                }
                }    
          productService.RetailerProductbyDepartment(deptdataRT).then(function (response) {
              console.log(response.data);           
              for (var i = 0; i < response.data.data.length; i++) {
                if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }        
             }
             var dataobject={
              "method":"retailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }

              if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
           }, function (response) {
          console.log(response);
        }
       );
    }
    else{
          //console.log('coming to here after selecting');
          sessionStorage.user=$scope.selecteRecord.retailerId;
          $scope.storeId = $scope.selecteRecord.storeid;
          sessionStorage.storeId = $scope.storeId;
              var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                //"items.mfgId":[$scope.mfgId],
                "item.categoryDesc": [$scope.categoryalreadyselected]
                }
              }    
          productService.RetailerProductbyStore(deptdataRT).then(function (response) {
              console.log(response.data);           
          for (var i = 0; i < response.data.data.length; i++) {
          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }        
             }

             var dataobject={
              "method":"store",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.storeId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
          }

             if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
          console.log(response);
        }
       );
      }
      }
    }
      }
    else {
       var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                     // "items.mfgId":[$scope.mfgId],
                     "item.categoryDesc": [$scope.categoryalreadyselected]
                    }
                  }    
          productService.getitemsbycategory(deptdataRT).then(function (response) {
          console.log(response.data);           
          for (var i = 0; i < response.data.data.length; i++) {
            if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
          var productobject = {
              "productName": $scope.productName,
              "itemName": response.data.data[i].upc,
              "deptId": response.data.data[i].deptId,
              "status": "plus"
            }
      if($scope.checkbeforeinsert(productobject)){
        $scope.productList.push(productobject);
      }
      }
        console.log("$scope.productList", $scope.productList);
        var dataobject={
              "method":"allretailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
              }
          if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
          console.log(response);
        }
       );
      }
       }
      }

      function daterangepickerCallBackreporttimeindividualInit(start, end) {
        $('#reporttime-individual span').html(start.format('MM/DD/YYYY') + ' - ' + end.format('MM/DD/YYYY'));

        startDate = start;
        endDate = end;
        $scope.ReportstartDate = startDate;
        $scope.Reportenddate = endDate;
        $scope.ReportstartDate = start.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.Reportenddate = end.format('YYYYMMDD') + 'T235959.000-0000';
        $scope.SalesDataReportstartDate =start.format('YYYYMMDD') + 'T000000.000-0000';
        $scope.ReportTimePeriodp = start.format('MMM DD YYYY') + ' - ' + end.format('MMM DD YYYY');
        $scope.totalreporttime=start.format('MMM Do YYYY')+"-"+end.format('MMM Do YYYY');
        $scope.$applyAsync();
        
        $scope.reportdateobject={
          "reportstartdate":$scope.ReportstartDate,
          "reportenddate": $scope.Reportenddate,
          "comparestartdate":$scope.ComparestartDate
        }
      }
      if(dashBoardService.gettimeperiodlabel()==undefined){
      dashBoardService.settimeperiodlabel("Last week");
     }

      $timeout(function() {
      $('#reporttime-individual').daterangepicker({
         maxDate: new Date(),
        startDate: start,
        endDate: end,
        "autoUpdateInput": false,
        "opens":"center",
        ranges: {
          'This Month': [moment().startOf('month'), moment().subtract(1,'days')],
          'Last Month': [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')],
          'Quarter 1 (Jan-Mar)': [moment().startOf('year').startOf('month'), moment().startOf('year').add(2, 'month').endOf('month')],
          'Quarter 2 (Apr-jun)': [moment().startOf('year').add(3, 'month').startOf('month'), moment().startOf('year').add(5, 'month').endOf('month')],
          'Quarter 3 (Jul-sep)': [moment().startOf('year').add(6, 'month').startOf('month'), moment().startOf('year').add(8, 'month').endOf('month')]
        }
      }, daterangepickerCallBackreporttimeindividual)}, 10);

      daterangepickerCallBackreporttimeindividualInit(start, end);

      $scope.getRecord = function () {
        if ($scope.selectedStoreId == 'allStores') {
          //$scope.SalesPerformanceByAllstores();
        }
        else {
          dashBoardService.setstatus(false);
          $scope.storeId = $scope.selectedStoreId;
          sessionStorage.storeId = $scope.storeId;
          $timeout(function () {
            $scope.TopProductsByStoreId();
            $scope.TopDepartmentsByStoreId();
          }, 0);
        }
      }

 
    $scope.getSelectedProduct = function (id) {
      $scope.productList = [];     
      //console.log(id);
      $scope.data.selectedStoreId=id;
      sessionStorage.storeId=id;
      //console.log($scope.data.selectedStoreId);
      if ($scope.data.selectedStoreId == ""|| $scope.data.selectedStoreId=="ALL STORES") {               
          dashBoardService.setstoreid("");
          dashBoardService.setdashboardcacheStaus(false);
          dashBoardService.setproductscacheStatus(false);
          dashBoardService.setsalesregioncacheStatus(false);
          var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                "item.deptId": $scope.optionid,
                "item.categoryDesc": $scope.category 
                }
              }    
          productService.RetailerProductbyDepartment(deptdataRT).then(function (response) {
          console.log(response.data);           
          for (var i =0;i<response.data.data.length;i++){
          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
          var productobject = {
              "productName": $scope.productName,
              "itemName": response.data.data[i].upc,
              "deptId": response.data.data[i].deptId,
              "status": "plus"
              }
          if($scope.checkbeforeinsert(productobject)){
          $scope.productList.push(productobject);
          }            
          }
          var dataobject={
              "method":"allstores",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
           }

            if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }

          }, function (response) {
            console.log(response);
          }
          );
       }
      else{
         var deptdataRT ={
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
            "item.deptId": $scope.optionid,
            "item.categoryDesc": $scope.category 
            }
          }
        productService.RetailerProductbyStore(deptdataRT).then(function (response) {
        console.log(response.data);           
        for (var i = 0; i < response.data.data.length; i++) {
           if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
        var productobject = {
            "productName": $scope.productName,
            "itemName": response.data.data[i].upc,
            "deptId": response.data.data[i].deptId,
            "status": "plus"
            }
        if($scope.checkbeforeinsert(productobject)){
        $scope.productList.push(productobject);
        } 
        }
         console.log("$scope.productList", $scope.productList);
          dashBoardService.setstoreid($scope.data.selectedStoreId);
          dashBoardService.setdashboardcacheStaus(false);
          dashBoardService.setproductscacheStatus(false);
          dashBoardService.setsalesregioncacheStatus(false);
          $scope.storeId = $scope.data.selectedStoreId;
          sessionStorage.storeId = $scope.storeId;
          var dataobject={
              "method":"store",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
             }

             if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }

         }, function (response) {
            console.log(response);
          }
          );
       }
    }

         $scope.role=sessionStorage.role;
         $scope.mfgId=sessionStorage.mfgId;


      $scope.init = function () {
        if($scope.role=="retailer"){
         $scope.GetStoreList();
         /*$timeout(function(){
           $scope.selectdropdown();
         },1000)*/
       
        $scope.selectdepartmentProducts();
         $scope.getDepartmentList(); 
      }

      else if($scope.role=="cpg"){
        $scope.getAllRetailers();
        //$scope.getDmaList();
        //$scope.getcategoriesList();
        $scope.selectdepartmentProductsbycpg();
        $scope.getallitems();   

              // $scope.selectedcpg=dashBoardService.getsavestoreselected();           
              // $scope.selecteRecord= $scope.selectedcpg; 
              // $scope.selecteddma=dashBoardService.getselectedDMA();           
              // $scope.savedBrand=dashBoardService.getselectedBrand();
              // $scope.selectedBrand= $scope.savedBrand;            
              //   if( $scope.selecteddma!= null){
              //      $scope.dma= $scope.selecteddma.name;
              //      sessionStorage.user=$scope.selectedcpg.retailerId;
              //      $scope.retailerid=$scope.selectedcpg.retailerId;
              //      $scope.dmaStoreList=[];
              //        for(var i=0;i<$scope.selecteddma.stores.length;i++){
              //        $scope.dmaStoreList.push($scope.selecteddma.stores[i].toString());
              //      }    
              //  } 

      }
      else {
        $scope.getDistributorBrands();
        $scope.getcategoriesforAllBrands();
         $scope.getallitemsfordistributor();
      }
      }

      $scope.init();

      $scope.getjatStyle = function (value) {
        if (value >= 1) {
          return "page_dashboard_box_index green";
        } else {
          return "page_dashboard_box_index red";
        }
      };

 
    $scope.getcpgrecord =function() {
      document.getElementById("dmaValue").value ="";
       $scope.dma="";
       dashBoardService.saveselectedDMA(null);
    $scope.selecteRecord=dashBoardService.getselectedrecordfromdropdown();
        $scope.productList=[];
        console.log("getcpgRecord",$scope.selecteRecord);
    //  if($scope.selecteRecord.hasOwnProperty("dmaId")){
    //   $scope.dmaStoreList=[];
    //   for(var i=0;i<$scope.selecteRecord.stores.length;i++){
    //   $scope.dmaStoreList.push($scope.selecteRecord.stores[i].toString());
    //  }

    //   var deptdataRT ={
    //             "aggTimeUnit": "1d",
    //             "startTime": $scope.ReportstartDate,
    //             "endTime": $scope.Reportenddate,
    //             "bucketLevel" : "L",
    //             "bucketType" : "upc",              
    //             "filters": {
    //             "storeId": $scope.dmaStoreList,
    //             "item.categoryDesc": [$scope.categoryalreadyselected]
    //             }
    //             }    
    // productService.getitemsbycategory(deptdataRT).then(function (response) {
    //           console.log(response.data);           
    // for (var i =0;i<response.data.data.length;i++) {

    //   if(response.data.data[i].name){
    //       if(response.data.data[i].name!=""){
    //         $scope.productName=response.data.data[i].name;
    //       }
    //       else{
    //          $scope.productName=response.data.data[i].id;
    //       }
    //       }
    //       else{
    //         $scope.productName=response.data.data[i].id;
    //       }
    // var productobject = {
    //     "productName":$scope.productName,
    //     "itemName":response.data.data[i].upc,
    //     "deptId":response.data.data[i].deptId,
    //     "status": "plus"
    //     }
    //   if($scope.checkbeforeinsert(productobject)){
    //   $scope.productList.push(productobject);
    //     }           
    //     }
    //      console.log("$scope.productList", $scope.productList);
    //  var dataobject={
    //            "method":"dma",
    //            "compareStartDate":$scope.ComparestartDate,
    //            "compareEndDate":$scope.Compareenddate,
    //            "reportEndDate":$scope.Reportenddate,
    //            "reportStartDate":$scope.ReportstartDate,
    //            "storeid":$scope.dmaStoreList,
    //            "itemNumber":$scope.deptitemNameArray,
    //            "salesDatastartdate":$scope.SalesDataReportstartDate,
    //            "salesDataenddate":$scope.SalesDataComparestartDate
    //            }
    //   if($scope.selectedItemsDataAraayforbarchart.length>0){
    //   for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
    //       var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
    //       console.log("results...",results);
    //       if(results.length==0){
    //       var data ={
    //           "deptId":$scope.optionid,
    //           "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
    //           "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
    //           "status":"minus"
    //       }
    //         $scope.productList.push(data);
    //       }
    //       for(var j=0;j<$scope.productList.length;j++){
    //        if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
    //         $scope.productList[j].status="minus";
    //        }
    //       } 
    //       }
    //       if($scope.selectedgraph =='line'){
    //         $rootScope.$emit('LinechartEvent', dataobject);
    //         $scope.linechartapicallstatus=true;
    //         $scope.barchartapicallstatus=false;
    //         //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
    //       }
    //       else{
    //         $rootScope.$emit('myCustomEvent', dataobject);
    //         $scope.barchartapicallstatus=true;
    //         $scope.linechartapicallstatus=false;
    //       }
    //       }
    //     }, function (response) {
    //       console.log(response);
    //     }
    //    );
    //  }
    //  else{
        if($scope.selecteRecord.name=="All Retailers"){

          $scope.rids=undefined;
        for(var i=0;i<$scope.retailerList.length;i++){
        if($scope.rids){
        $scope.rids=$scope.rids+","+$scope.retailerList[i].retailer_id;
        }
        else{
        $scope.rids=$scope.retailerList[i].retailer_id;
        }
        }
        sessionStorage.rids=$scope.rids;


            var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                //"items.mfgId":[$scope.mfgId],
                "retailerId" :$scope.RetailerIds,
                "item.categoryDesc": [$scope.categoryalreadyselected]
                }
               }    
          productService.getitemsbycategory(deptdataRT).then(function (response) {
          console.log(response.data);           
          for(var i =0;i<response.data.data.length;i++){
          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }        
             }
             console.log("$scope.productList", $scope.productList);
           var dataobject={
               "method":"allretailer",
               "compareStartDate":  $scope.ComparestartDate,
               "compareEndDate":$scope.Compareenddate,
               "reportEndDate":$scope.Reportenddate,
               "reportStartDate":$scope.ReportstartDate,
               "storeid":$scope.data.selectedStoreId,
               "itemNumber":$scope.deptitemNameArray,
               "salesDatastartdate":$scope.SalesDataReportstartDate,
               "salesDataenddate":$scope.SalesDataComparestartDate
               }
         if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
          console.log(response);
        }
       );

            
        }
        else{
            if($scope.selecteRecord.hasOwnProperty("children")){
                sessionStorage.user=$scope.selecteRecord.retailerId;
                $scope.retailerid=$scope.selecteRecord.retailerId;

               var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                     // "items.mfgId":[$scope.mfgId],
                     "item.categoryDesc": [$scope.categoryalreadyselected]
                    }
                  }    
          productService.RetailerProductbyDepartment(deptdataRT).then(function (response) {
              console.log(response.data);           
              for (var i = 0; i < response.data.data.length; i++) {

                if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
                         
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }        
             }
              var dataobject={
                            "method":"retailer",
                             "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
                if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
            $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            var productclicked= $rootScope.$on('chartdata', function (event, data) {
                  $scope.itemsList[0].linechart.data= data
                  // console.log(" $scope.itemsList", $scope.itemsList);
               });
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }


           }, function (response) {
          console.log(response);
        }
       );
            }
            else{
                sessionStorage.user=$scope.selecteRecord.retailerId;
                $scope.storeId = $scope.selecteRecord.storeid;
                sessionStorage.storeId = $scope.storeId;

                 var deptdataRT ={
                "aggTimeUnit": "1d",
                "startTime": $scope.ReportstartDate,
                "endTime": $scope.Reportenddate,
                "bucketLevel" : "L",
                "bucketType" : "upc",
                "filters": {
                     // "items.mfgId":[$scope.mfgId],
                     "item.categoryDesc": [$scope.categoryalreadyselected]
                    }
                  }    
          productService.RetailerProductbyStore(deptdataRT).then(function (response) {
              console.log(response.data);           
          for (var i =0;i<response.data.data.length;i++) {

          if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }
              var productobject = {
                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus"
                  }
                         
              if($scope.checkbeforeinsert(productobject)){
                $scope.productList.push(productobject);
              }        
             }

        var dataobject={
                  "method":"store",
                  "compareStartDate":  $scope.ComparestartDate,
                  "compareEndDate":$scope.Compareenddate,
                  "reportEndDate":$scope.Reportenddate,
                  "reportStartDate":$scope.ReportstartDate,
                  "storeid":$scope.storeId,
                  "itemNumber":$scope.deptitemNameArray,
                  "salesDatastartdate":$scope.SalesDataReportstartDate,
                  "salesDataenddate":$scope.SalesDataComparestartDate
                }

          if($scope.selectedItemsDataAraayforbarchart.length>0){
          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
          var results=$filter('filter')($scope.productList,{itemName:$scope.selectedItemsDataAraayforbarchart[i].itemNumber}, true);
          console.log("results...",results);
          if(results.length==0){
          var data ={
              "deptId":$scope.optionid,
              "itemName":$scope.selectedItemsDataAraayforbarchart[i].itemNumber,
              "productName":$scope.selectedItemsDataAraayforbarchart[i].productName,
              "status":"minus"
          }
          $scope.productList.push(data);
          }
          for(var j=0;j<$scope.productList.length;j++){
           if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==$scope.productList[j].itemName){
            $scope.productList[j].status="minus";
           }
          } 
          }
          if($scope.selectedgraph =='line'){
            $rootScope.$emit('LinechartEvent', dataobject);
            $scope.linechartapicallstatus=true;
            $scope.barchartapicallstatus=false;
            var productclicked= $rootScope.$on('chartdata', function (event, data) {
                  $scope.itemsList[0].linechart.data= data
                  // console.log(" $scope.itemsList", $scope.itemsList);
               });
            //$scope.getproductBySelectedItem($scope.selectedItemsDataAraayforbarchart[i].itemNumber,$scope.selectedItemsDataAraayforbarchart[i].productName);
          }
          else{
            $rootScope.$emit('myCustomEvent', dataobject);
            $scope.barchartapicallstatus=true;
            $scope.linechartapicallstatus=false;
          }
          }
          }, function (response) {
          console.log(response);
        }
       );                
      }
      }
    // }
    };

// collapse function
      // for linechart

       // $scope.arrowClass = "glyphicon-arrow-down";
        // $scope.changeArrowAvgBasket = function(itemNumber){


         /* if($scope.arrowClass === "glyphicon-arrow-down")
          {
            $scope.arrowClass = "glyphicon-arrow-up";
          } 
          else
          {
            $scope.arrowClass = "glyphicon-arrow-down";
          }
*/

  // for barchart
        // $scope.lineChartArrow = "glyphicon-arrow-up";
        // $scope.lineChartArrowClick = function(itemnumber){


          /*if($scope.lineChartArrow === "glyphicon-arrow-down")
          {
            $scope.lineChartArrow = "glyphicon-arrow-up";
          } 
          else
          {
            $scope.lineChartArrow = "glyphicon-arrow-down";
          }*/




// collapse function
      // for linechart
     $scope.changeArrowAvgBasket = function(itemNumber){
     // console.log("record...",$scope.selectedItemsDataAraayforlinechart);

       for(var i=0;i<$scope.selectedItemsDataAraayforlinechart.length;i++){
          if($scope.selectedItemsDataAraayforlinechart[i].itemNumber==itemNumber){

           if($scope.selectedItemsDataAraayforlinechart[i].arrowdircetion === "glyphicon-arrow-down"){
               $scope.selectedItemsDataAraayforlinechart[i].arrowdircetion="glyphicon-arrow-up";
              }
           else{
              $scope.selectedItemsDataAraayforlinechart[i].arrowdircetion="glyphicon-arrow-down";

              }

          }
          }

        }

          // for barchart

  $scope.lineChartArrowClick = function(itemnumber){
    for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
       if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==itemnumber){

     if($scope.selectedItemsDataAraayforbarchart[i].arrowdircetion === "glyphicon-arrow-up"){
         $scope.selectedItemsDataAraayforbarchart[i].arrowdircetion="glyphicon-arrow-down";
       }
              else{
  $scope.selectedItemsDataAraayforbarchart[i].arrowdircetion="glyphicon-arrow-up";

              }

          }
         }

         }


         $scope.reporttimeranges=[
                              {"name":"LAST 7 DAYS"},
                              {"name":"THIS MONTH"},
                              {"name":"LAST MONTH"},
                              {"name":"QUARTER 1 2017"},
                              {"name":"QUARTER 2 2017"},
                              {"name":"QUARTER 3 2017"},
                              {"name":"QUARTER 4 2017"}
                              
                              ];

        $scope.selectedtimeperiod=$scope.reporttimeranges[0];
       $scope.timeperiodlabel=dashBoardService.gettimeperiodlabel();
            
            if($scope.timeperiodlabel){
              if($scope.timeperiodlabel=="Last week"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[0];
            }
             else if($scope.timeperiodlabel=="This Month"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[1];
            }
            else if($scope.timeperiodlabel=="Last Month"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[2];
            }
             
            else if($scope.timeperiodlabel=="Custom Range"){

              if($scope.reporttimeranges[7]==undefined){

                var obj={"name":"CUSTOM"}
                 $scope.reporttimeranges.push(obj);
             }

              $scope.selectedtimeperiod=$scope.reporttimeranges[7];
            }
            else if($scope.timeperiodlabel=="Quarter 1 (Jan-Mar)"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[3];
            }

             else if($scope.timeperiodlabel=="Quarter 2 (Apr-jun)"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[4];
            }
             else if($scope.timeperiodlabel=="Quarter 3 (Jul-sep)"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[5];
            }
            else if($scope.timeperiodlabel=="Quarter 4 (Oct-Dec)"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[6];
            }
            }
            else{
               $scope.selectedtimeperiod=$scope.reporttimeranges[0];
            }
             $scope.timerange=function(value){
                if($scope.reporttimeranges.indexOf(7)){
                 $scope.reporttimeranges.splice(7, 1);
              }
           
            if(value.name=="LAST 7 DAYS"){
              $scope.selectedtimeperiod=$scope.reporttimeranges[0];
              $scope.lastsevendays=true;
              $scope.starttime=moment().subtract(7,'days');
              $scope.endtime=moment().subtract(1,'days');
              daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Last week");
            }
            else if(value.name=="THIS MONTH"){

             $scope.selectedtimeperiod=$scope.reporttimeranges[1];

              $scope.starttime=moment().startOf('month');
              $scope.endtime=moment().subtract(1,'days');

              daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"This Month");

            }
            else if(value.name=="LAST MONTH"){
                $scope.selectedtimeperiod=$scope.reporttimeranges[2];

               $scope.starttime=moment().subtract(1,'month').startOf('month');
              $scope.endtime=moment().subtract(1,'month').endOf('month');

              daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Last Month");

            }
            else if(value.name=="QUARTER 1 2017"){

          $scope.selectedtimeperiod=$scope.reporttimeranges[3];

               $scope.starttime=  moment().startOf('year').startOf('month');
              $scope.endtime= moment().startOf('year').add(2, 'month').endOf('month');
           daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Quarter 1 (Jan-Mar)");

            }
            else if(value.name=="QUARTER 2 2017"){
               $scope.selectedtimeperiod=$scope.reporttimeranges[4];
            
              $scope.starttime=moment().startOf('year').add(3, 'month').startOf('month'); 
              $scope.endtime=moment().startOf('year').add(5, 'month').endOf('month');
           daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Quarter 2 (Apr-jun)");

            }
            else if(value.name=="QUARTER 3 2017"){

           $scope.selectedtimeperiod=$scope.reporttimeranges[5];
   
          $scope.starttime=moment().startOf('year').add(6, 'month').startOf('month');
            $scope.endtime=moment().startOf('year').add(8, 'month').endOf('month');

           daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Quarter 3 (Jul-sep)");

            }
              else if(value.name=="QUARTER 4 2017"){

            $scope.selectedtimeperiod=$scope.reporttimeranges[6];
           
          $scope.starttime=moment().startOf('year').add(9, 'month').startOf('month');
            $scope.endtime=moment().subtract(1,'days');;

           daterangepickerCallBackreporttimeindividual($scope.starttime,$scope.endtime,"Quarter 4 (Oct-Dec)");

            }
            
          }



 $scope.selectedgraph ='line';

      $scope.selectgraph=function (graph,itemNumber) {

        $scope.selectedgraph =graph;



      if(graph=="line"){
 /*for(var i=0;i<$scope.selectedItemsDataAraayforlinechart.length;i++){
      if($scope.selectedItemsDataAraayforlinechart[i].itemNumber==itemNumber){
          
          $scope.selectedItemsDataAraayforlinechart[i].linechart=true;
             

          }
          }

          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
       if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==itemNumber){

         $scope.selectedItemsDataAraayforbarchart[i].barchart=false;
          }
         }*/

          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==itemNumber){
            $scope.itemsList[i].linechart.linechart=true;
            $scope.itemsList[i].barchart.barchart=false;

          }
         }



      }
      else{

       /*  for(var i=0;i<$scope.selectedItemsDataAraayforlinechart.length;i++){
      if($scope.selectedItemsDataAraayforlinechart[i].itemNumber==itemNumber){
          
          $scope.selectedItemsDataAraayforlinechart[i].linechart=false;
             

          }
          }

          for(var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
       if($scope.selectedItemsDataAraayforbarchart[i].itemNumber==itemNumber){

         $scope.selectedItemsDataAraayforbarchart[i].barchart=true;
          }
         }*/

          for(var i=0;i<$scope.itemsList.length;i++){
          if($scope.itemsList[i].itemNumber==itemNumber){
            $scope.itemsList[i].linechart.linechart=false;
            $scope.itemsList[i].barchart.barchart=true;

          }
         }

      }

   



        if($scope.selectedgraph =='line'){
          if($scope.linechartapicallstatus==false){
             if($scope.selectedItemsDataAraayforlinechart.length>0){
              if($scope.role=="retailer"){
             if ($scope.data.selectedStoreId == ""|| $scope.data.selectedStoreId=="ALL STORES") {
               var dataobject={
                  "method":"allstores",
                 "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;
              //$scope.barchartapicallstatus=false;


             }
             else{

               var dataobject={
                 "method":"store",
                "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }

              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;
              //$scope.barchartapicallstatus=false;


              }
             }
             else if($scope.role=="distributor"){
                 var dataobject={
                             "compareStartDate":$scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;
             }

             else if($scope.role=="cpg"){


              $scope.selecteRecord=   dashBoardService.getselectedrecordfromdropdown();
              if($scope.selecteRecord){

                 if($scope.selecteRecord.hasOwnProperty("dmaId")){
      $scope.dmaStoreList=[];
      for(var i=0;i<$scope.selecteRecord.stores.length;i++){
      $scope.dmaStoreList.push($scope.selecteRecord.stores[i].toString());
       }
        var dataobject={"method":"dma",
                       "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.dmaStoreList,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;




     }
     else{



          if($scope.selecteRecord.name=="All Retailers"){
                       var dataobject={"method":"allretailer",
                       "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;
         }
         else{
          if($scope.selecteRecord.hasOwnProperty("children")){
           sessionStorage.user=$scope.selecteRecord.retailerId;
           $scope.retailerid=$scope.selecteRecord.retailerId;

                var dataobject={  "method":"retailer",
                 "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;

         }
         else{

          sessionStorage.user=$scope.selecteRecord.retailerId;
          $scope.storeId = $scope.selecteRecord.storeid;
          sessionStorage.storeId = $scope.storeId;
         var dataobject={ 
                        "method":"store",
                       "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.storeId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;

          
        }
      }
    }
    }
    else {
         var dataobject={"method":"allretailer",
                       "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('LinechartEvent', dataobject);
              $scope.linechartapicallstatus=true;
    }

             }
           }

             }

        }else if($scope.selectedgraph =='bar'){
          if($scope.barchartapicallstatus==false){
          if($scope.selectedItemsDataAraayforbarchart.length>0){
            if($scope.role=="retailer"){
          if ($scope.data.selectedStoreId == ""|| $scope.data.selectedStoreId=="ALL STORES") {
            var dataobject={
               "method":"allstores",
              "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;
          // $scope.linechartapicallstatus=false;


          }
          else{

            var dataobject={
              "method":"store",
              "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }

           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;
        //   $scope.linechartapicallstatus=false;

           }
          }

          else if($scope.role=="distributor"){
             var dataobject={
                           "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;
          }

          else if($scope.role=="cpg"){
            $scope.selecteRecord=   dashBoardService.getselectedrecordfromdropdown();
           if($scope.selecteRecord){

            if($scope.selecteRecord.hasOwnProperty("dmaId")){
      $scope.dmaStoreList=[];
      for(var i=0;i<$scope.selecteRecord.stores.length;i++){
      $scope.dmaStoreList.push($scope.selecteRecord.stores[i].toString());
       }
        var dataobject={"method":"dma",
                       "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.dmaStoreList,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
              $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;
        }
     else{
          if($scope.selecteRecord.name=="All Retailers"){
var dataobject={
               "method":"allretailer",
              "compareStartDate":  $scope.ComparestartDate,
              "compareEndDate":$scope.Compareenddate,
              "reportEndDate":$scope.Reportenddate,
              "reportStartDate":$scope.ReportstartDate,
              "storeid":$scope.data.selectedStoreId,
              "itemNumber":$scope.deptitemNameArray,
              "salesDatastartdate":$scope.SalesDataReportstartDate,
              "salesDataenddate":$scope.SalesDataComparestartDate
            }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;

         }
         else{
          if($scope.selecteRecord.hasOwnProperty("children")){
           sessionStorage.user=$scope.selecteRecord.retailerId;
           $scope.retailerid=$scope.selecteRecord.retailerId;
var dataobject={
               "method":"retailer",
              "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;


         }
         else{

          sessionStorage.user=$scope.selecteRecord.retailerId;
          $scope.storeId = $scope.selecteRecord.storeid;
          sessionStorage.storeId = $scope.storeId;
          
var dataobject={
               "method":"store",
              "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.storeId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;

        }
      }
    }
          }
          else {
         var dataobject={
               "method":"allretailer",
              "compareStartDate":  $scope.ComparestartDate,
                             "compareEndDate":$scope.Compareenddate,
                             "reportEndDate":$scope.Reportenddate,
                             "reportStartDate":$scope.ReportstartDate,
                             "storeid":$scope.data.selectedStoreId,
                             "itemNumber":$scope.deptitemNameArray,
                             "salesDatastartdate":$scope.SalesDataReportstartDate,
                             "salesDataenddate":$scope.SalesDataComparestartDate
                           }
           $rootScope.$emit('myCustomEvent', dataobject);
           $scope.barchartapicallstatus=true;
          }
        }
        }

        
      }


      }
    }


// Print function.......


$scope.printToCart = function(printSectionId) {
    
  $scope.indiproductObjectdata={

         "selectedOption":$scope.selectedOption,
         "selectedvalue1":$scope.selectedvalue1,
        "selectedProduct":$scope.selectedProduct,
        "selectedtimeperiod":$scope.selectedtimeperiod,
        "ReportTimePeriodp": $scope.ReportTimePeriodp,
        "compareTimePeriod":$scope.compareTimePeriod,
        "itemsList":$scope.itemsList,
        "role":$scope.role,
        "productList":$scope.productList,
        "reportdateobject":$scope.reportdateobject,
        "selectedcategory":$scope.selectedcategory
           

       };

       console.log('printdata',$scope.indiproductObjectdata);



productService.setindividualproduct($scope.indiproductObjectdata);
 $state.go('individualPrdct-print');

};


$scope.gotoProductdetail=function(product){
 console.log("product",product);
 sessionStorage.selectedUPC=product;
  for(var i=0; i<$scope.productList.length; i++){
    if(product == $scope.productList[i].itemName){ 
      $scope.productList[i].status="plus";    
       productService.setproductdetail($scope.productList[i]);
       if($scope.role=='retailer'){
          productService.DeptDataforRetailer().then(function (response) {            
            productService.setCategoryforRetailer(response.data[0].categoryName);
            $state.go('Product-detail');
          });
       }else 
       if($scope.role=='cpg'){
           productService.DeptDataforCPG().then(function (response) {
            productService.setselectedCategory(response.data[0].categoryName);
            $state.go('Product-detail');
          });
       }
       else if($scope.role=='distributor'){
           productService.DeptDataforDistribiter().then(function (response) {
            productService.setselectedCategory(response.data[0].categoryName);
            $state.go('Product-detail');
          });
       }
              
    }
    
  }
 }
 $scope.removePrdct=function(data){
   for (var i = 0; i < $scope.productList.length; i++) {
       if ($scope.productList[i].itemName == data) {
            $scope.productList[i].status = "plus"
         }
  }

  for (var i=0;i<$scope.selectedItemsDataAraayforbarchart.length;i++){
      if ($scope.selectedItemsDataAraayforbarchart[i].itemNumber == data) {
          $scope.selectedItemsDataAraayforbarchart.splice(i, 1);
       }
  }

  for (var i =0;i<$scope.selectedItemsDataAraayforlinechart.length;i++){
      if ($scope.selectedItemsDataAraayforlinechart[i].itemNumber == data) {
           $scope.selectedItemsDataAraayforlinechart.splice(i, 1);
      }
 }

  for(var j=0;j<$scope.itemsList.length;j++){
      if($scope.itemsList[j].itemNumber==data){        
        $scope.itemsList.splice(j, 1);
       }
        
  }
     for(var j=0;j<$scope.upcDataArr.length;j++){
         if($scope.upcDataArr[j].UPC==data){
           $scope.upcArr.splice(j, 1);
        }
      }
}
        
 
 


 /*************dma api calls************/

 $scope.subproductListbyDMA = function (deptid) {

      sessionStorage.itemssize=500;

        $scope.deptmentid = deptid;

          var deptdataRT =
          {
            "aggTimeUnit": "1d",
            "startTime": $scope.ReportstartDate,
            "endTime": $scope.Reportenddate,
            "bucketLevel" : "L",
            "bucketType" : "upc",
            "filters": {
              "item.categoryDesc" : [deptid],
              // "items.mfgId" : [$scope.mfgId],
              "storeId": $scope.dmaStoreList
            }
          }

          productService.getitemsbycategory(deptdataRT).then(function (response) {

              $scope.subproductlists = [];
              $scope.productList = [];

              for (var i = 0; i < response.data.data.length; i++) {

                if(response.data.data[i].name){
          if(response.data.data[i].name!=""){
            $scope.productName=response.data.data[i].name;
          }
          else{
             $scope.productName=response.data.data[i].id;
          }
          }
          else{
            $scope.productName=response.data.data[i].id;
          }

                  var object = {

                    "itemName": response.data.data[i].upc,
                    "name": $scope.productName,
                    "deptId": response.data.data[i].deptId,
                    "amt": response.data.data[i].amt,
                    "cp": response.data.data[i].cp,
                    "namecp": response.data.data[i].name,
                    "category":deptid
                  };

                  var productobject = {

                    "productName": $scope.productName,
                    "itemName": response.data.data[i].upc,
                    "deptId": response.data.data[i].deptId,
                    "status": "plus",
                     "category":deptid
                  }
                  $scope.productList.push(productobject);
                  $scope.subproductlists.push(object);

                
              }

            // for(var i=0; i< $scope.getcompproduct.length; i++){
              if($scope.individualcategoryproduct==true){

                 $scope.optionpname   = window.localStorage['pname'];
               $scope.option = window.localStorage['selectoption1'];
               $scope.optionid = window.localStorage['iddept'];
               $scope.dataplusmark=window.localStorage['dataplus'];
               $scope.optionpitemNumber   =  window.localStorage['itemNum'];
                
                 var data ={
                  "deptId":$scope.optionid,
                  "itemName":$scope.optionpitemNumber,
                  "productName":$scope.optionpname,
                   "status":"plus"
                 }
                   
                if($scope.getcompproduct == null){
                     $scope.selectedProduct(data);
                 }else{
                  for(var i=0; i< $scope.getcompproduct.length; i++){
                  $scope.selectedProduct($scope.getcompproduct[i]);
                }
               }
                               
              }
              else{
                 if($scope.getcompproduct == null){
                     $scope.selectedProduct($scope.productList[0]);
                 }else{
                  for(var i=0; i< $scope.getcompproduct.length; i++){
                     $scope.selectedProduct($scope.getcompproduct[i]);
                   }
                 }
              
              }

            
            }, function (response) {
              console.log(response);
            }
          );
           
      }

       /*$scope.getproductBySelectedItemforDMA = function (deptmName,productname) {
        $scope.getproductforRTforDMA = function () {
        var data1 = {
            "aggTimeUnit": "1d",
            "startTime": $scope.SalesDataComparestartDate,
            "endTime": $scope.Compareenddate,
            "filters": {
            "storeId": $scope.dmaStoreList,
            "item.upc": [deptmName],
            "items.mfgId" : [$scope.mfgId],
            "item.categoryDesc" : [$scope.selectedcategory.category_description]
            }
            }

        productService.GetSalesPerformanceByAllRetailerswithoutsize(data1).then(function (response) {

          $scope.ProductsbyCT = response.data.data;
          if(response.data){
            $scope.unitsoldbycp=response.data.totalSoldQty
          }
        $scope.spIndextotal = parseFloat(response.data.total);
        $scope.$applyAsync();
         var data = {
            "aggTimeUnit": "1d",
            "startTime": $scope.SalesDataReportstartDate,
            "endTime": $scope.Reportenddate,
            "filters":{
            "storeId": $scope.dmaStoreList,
            "item.upc": [deptmName],
            "items.mfgId" : [$scope.mfgId],
            "item.categoryDesc" : [$scope.selectedcategory.category_description]
           }
          }

          $scope.showproducts=false;
          $scope.spin=true;
        productService.GetSalesPerformanceByAllRetailerswithoutsize(data).then(function (response) {
          $scope.ProductsbyRT = response.data.data;
          $scope.singleProductchartId = productService.generateguid();
          $scope.rpIndextotal = parseFloat(response.data.total);
          $scope.totalunitsold=response.data.totalSoldQty;

          var unitsold={
            "unitSoldbyRp":$scope.totalunitsold,
            "unitSoldbyCp":$scope.unitsoldbycp
            }


          if($scope.spIndextotal==0||$scope.spIndextotal==null) {
            $scope.spIndex = 0;
          }
          else {
            $scope.spIndex =$scope.rpIndextotal/$scope.spIndextotal;
                        $scope.spIndex =$scope.spIndex.toFixed(2);
          }

          $scope.barchartData = [];

                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                     
                      }

                      $scope.barchartData.push(barchartDataObject);

                      var barchartObject = {
                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }

                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);

                     $scope.shareOfCategoryByAllRetailers(deptmName);
                      $scope.ShareOfBasketByAllRetailer(deptmName);

                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];

 var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
              var days = b.diff(a, 'days');


        $scope.j=1;

         for (var i = 0; i <= days; i++) {

             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
              $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");

             }
             else{
               
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");;
                $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");;

               $scope.j++;

             }

              //here

                  if($scope.ProductsbyCT&&$scope.ProductsbyRT){

              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){

                var linechartdataobject = {

                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);

                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                  
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                       "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }

                      $scope.linechartData.push(linechartdataobject);

                    }

                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }

                      $scope.linechartData.push(linechartdataobject);

                    }

              }
              else{
                if($scope.ProductsbyRT[i]){

                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                              "avgbasket":"",
                              "unitsold":unitsold,
                              "arrowdircetion":"glyphicon-arrow-down",
                              "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);

                            var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }

                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                            }, function (response) {
                              console.log(response);
                            }
                          );
                       
                          }, function (response) {
                            console.log(response);
                          }
                        );
                        }
                      $scope.getproductforRTforDMA();
                      }*/



                $scope.getproductBySelectedItemforDMA = function (deptmName,productname) {
                $scope.getproductforRTforDMA = function () {

                   var data1 = {
                          "aggTimeUnit": "1d",
                          "startTime": $scope.SalesDataComparestartDate,
                          "endTime": $scope.Compareenddate,
                          "filters": {
                            "storeId": $scope.dmaStoreList,
                            "item.upc": [deptmName]
                            // "items.mfgId" : [$scope.mfgId],
                            // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                          }
                        }

                        productService.GetSalesPerformanceByAllRetailerswithoutsize(data1).then(function (response) {
                            $scope.ProductsbyCT = response.data.data;
                            if(response.data){
                              $scope.unitsoldbycp=response.data.totalSoldQty
                            }
                          $scope.spIndextotal = parseFloat(response.data.total);
                        
                            $scope.$applyAsync();

         var data = {
                     "aggTimeUnit": "1d",
                     "startTime": $scope.SalesDataReportstartDate,
                     "endTime": $scope.Reportenddate,
                     "filters":{
                      "storeId": $scope.dmaStoreList,
                     "item.upc": [deptmName]
                     // "items.mfgId" : [$scope.mfgId],
                     // "item.categoryDesc" : [$scope.selectedcategory.category_description]
                     }
                    }

                    $scope.showproducts=false;
                    $scope.spin=true;
                    productService.GetSalesPerformanceByAllRetailerswithoutsize(data).then(function (response) {
                      $scope.ProductsbyRT = response.data.data;

                      $scope.singleProductchartId = productService.generateguid();
                      $scope.rpIndextotal = parseFloat(response.data.total);

                      $scope.totalunitsold=response.data.totalSoldQty;

                        var unitsold={
                          "unitSoldbyRp":$scope.totalunitsold,
                          "unitSoldbyCp":$scope.unitsoldbycp
                         }

                      if ($scope.spIndextotal == 0 || $scope.spIndextotal == null) {
                        $scope.spIndex = 0;
                      }
                      else {
                        $scope.spIndex = $scope.rpIndextotal / $scope.spIndextotal;
                        $scope.spIndex = $scope.spIndex.toFixed(2);
                      }
                      $scope.barchartData = [];
                      var barchartDataObject = {
                        "productName": productname,
                        "name": deptmName,
                        "income": $scope.rpIndextotal,
                        "expenses": $scope.spIndextotal,
                        "color": "#4C98CF",
                        "color1": "#7F2891",
                        "unitSoldbyRp":$filter('number')($scope.totalunitsold,2),
                        "unitSoldbyCp": $filter('number')($scope.unitsoldbycp,2),
                        "reporttime":$scope.totalreporttime,
                        "comapretime":$scope.totalcomparetime
                     
                      }

                      $scope.barchartData.push(barchartDataObject);

                      var barchartObject = {

                        "id": $scope.singleProductchartId,
                        "data": $scope.barchartData,
                        "itemNumber": deptmName,
                        "index": $scope.spIndex,
                        "total": $scope.rpIndextotal,
                        "productName": productname,
                        "shoppingtrips":"",
                        "avgbasket":"",
                        "unitsold":unitsold,
                        "arrowdircetion":"glyphicon-arrow-up",
                        "barchart":false
                      }

                      $scope.selectedItemsDataAraayforbarchart.push(barchartObject);

                     $scope.shareOfCategoryByDMA(deptmName);
                      $scope.ShareOfBasketByDMA(deptmName);

                      $scope.singleProductLinechartId = productService.generateguid();
                      $scope.linechartData = [];

 var a = moment($scope.ReportstartDate);
                var b = moment($scope.Reportenddate);
              var days = b.diff(a, 'days');

        $scope.j=1;
         for (var i = 0; i <= days; i++) {
             if(i==0){
               $scope.date=moment($scope.ReportstartDate).format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.date).format("MM-DD-YYYY");
              $scope.ResultDate1=moment($scope.ComparestartDate).format("MM-DD-YYYY");
             }
             else{
               
               $scope.nextDate=moment($scope.date).add($scope.j,'days').format("YYYY-MM-DD");
               $scope.ResultDate=moment($scope.nextDate).format("MM-DD-YYYY");;
                $scope.ResultDate1=moment($scope.ComparestartDate).add(i,'days').format("MM-DD-YYYY");;
               $scope.j++;
             }
                  if($scope.ProductsbyCT&&$scope.ProductsbyRT){
              if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]){
                var linechartdataobject = {

                   "date": $scope.ResultDate,
                   "date1":$scope.ResultDate1,
                   "lineColor": "rgb(66, 141, 201)",
                   "value": $scope.ProductsbyRT[i].amt,
                   "value2": $scope.ProductsbyCT[i].amt,
                    "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                    "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                    "name": deptmName,
                    "productName": productname,
                    "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                    "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                     $scope.linechartData.push(linechartdataobject);
                    }
            else if($scope.ProductsbyRT[i]&&$scope.ProductsbyCT[i]==undefined){
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                  else if($scope.ProductsbyCT[i]&&$scope.ProductsbyRT[i]==undefined){
                  
                     var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0.00,
                        "value2":$scope.ProductsbyCT[i].amt,
                        "amt": 0.00,
                       "amt1": $filter('number')($scope.ProductsbyCT[i].amt,2),
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                         "unitSoldbyCp": $filter('number')($scope.ProductsbyCT[i].soldQuantity,2)
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
                    else{
                      var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": 0,
                        "value2":0,
                        "amt": 0.00,
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":0,
                        "unitSoldbyCp":0
                      }
                      $scope.linechartData.push(linechartdataobject);
                    }
              }
              else{
                if($scope.ProductsbyRT[i]){

                  var linechartdataobject = {
                        "date": $scope.ResultDate,
                        "date1":$scope.ResultDate1,
                        "lineColor": "rgb(66, 141, 201)",
                        "value": $scope.ProductsbyRT[i].amt,
                        "value2":0.00,
                       "amt": $filter('number')($scope.ProductsbyRT[i].amt,2),
                        "amt1":0.00,
                        "name": deptmName,
                        "productName": productname,
                         "unitSoldbyRp":$filter('number')($scope.ProductsbyRT[i].soldQuantity,2),
                         "unitSoldbyCp": 0
                      }
                      $scope.linechartData.push(linechartdataobject);
                }
              }
                  }
                            var linechartObject={
                              "id":$scope.singleProductLinechartId,
                              "data":$scope.linechartData,
                              "itemNumber":deptmName,
                              "index":$scope.spIndex,
                              "total":$scope.rpIndextotal,
                              "productName":productname,
                              "shoppingtrips":"",
                              "avgbasket":"",
                              "unitsold":unitsold,
                              "arrowdircetion":"glyphicon-arrow-down",
                              "linechart":true
                            }
                 $scope.selectedItemsDataAraayforlinechart.push(linechartObject);

                            var objectitems={
                              "barchart":barchartObject,
                              "linechart":linechartObject,
                              "itemNumber":deptmName
                            }

                          $scope.itemsList.push(objectitems);
                             $scope.showproducts=true;
                             $scope.spin=false;
                             $scope.masked = false;
                              $scope.$applyAsync();
                            }, function (response) {
                              console.log(response);
                            }
                          );
                       
                          }, function (response) {
                            console.log(response);
                          }
                        );
                        }
                            $scope.getproductforRTforDMA();
                      }


         $scope.shareOfCategoryByRTforDMA = function (itemnumber,avgBasketTotalByCP) {
            var ShoppingTripsdata = {

              "aggTimeUnit": "1d",
               "startTime":$scope.ReportstartDate,
              "endTime":$scope.Reportenddate,
              // "rid" : "000",
              "filters": {
                 "storeId" : $scope.dmaStoreList,
                "item.upc"  : [itemnumber]
              }
            }

              $scope.AvgBasket = false;
             
          productService.getAvgBasketforDistributor(ShoppingTripsdata).then(function (response) {

               $scope.avgBasketTotal = response.data.total;

          $scope.avgBasketTotalByRT = parseFloat(response.data.total);

          if (avgBasketTotalByCP == 0 || avgBasketTotalByCP == null || isNaN(avgBasketTotalByCP)) {
            $scope.ABtotal = 0;
            $scope.cpgavgbasketArr.push({UPC:itemnumber, ABtotal:$scope.ABtotal});
          }
          else {
            $scope.ABtotal = $scope.avgBasketTotalByRT /avgBasketTotalByCP;
            $scope.ABtotal = $scope.ABtotal.toFixed(2);
             $scope.cpgavgbasketArr.push({UPC:itemnumber, ABtotal:$scope.ABtotal});
          }

          for(var i=0;i<$scope.itemsList.length;i++){
             for(var j=0; j<$scope.cpgavgbasketArr.length; j++){
                if($scope.itemsList[i].itemNumber== $scope.cpgavgbasketArr[j].UPC){
                   $scope.itemsList[i].linechart.avgbasket=$scope.cpgavgbasketArr[j].ABtotal;
                   $scope.itemsList[i].barchart.avgbasket=$scope.cpgavgbasketArr[j].ABtotal;
            }
           }
         }

            }, function (response) {
              console.log(response);
            }
            );
          }
          


    $scope.shareOfCategoryByDMA= function (upc) {
      $scope.upcArr.push(upc);
          var ShoppingTripsdata = {

            "aggTimeUnit": "1d",
            "startTime": $scope.ComparestartDate,
            "endTime": $scope.Compareenddate,
            // "rid" : "000",
              "filters": {
                "storeId" : $scope.dmaStoreList,
               "item.upc"  : [upc]
              }
          }
          productService.getAvgBasketforDistributor(ShoppingTripsdata).then(function (response) {
           $scope.avgBasketTotalByCP = parseFloat(response.data.total);
            $scope.shareOfCategoryByRTforDMA(upc,$scope.avgBasketTotalByCP);

          }, function (response) {
            console.log(response);
          }
          );               
      }

       $scope.ShareOfBasketByRTDMA= function (itemnumber,ShoppingTripsCPindex) {


            var ShoppingTripsdata = {
               "aggTimeUnit": "1d",
               "startTime":$scope.ReportstartDate,
               "endTime":$scope.Reportenddate,
               // "rid" : "000",
                "filters": {
                "terms":{
                "storeId" : $scope.dmaStoreList,
                "item.upc"  : [itemnumber]
              }
            }
            }

            productService.getShoppingTripsforDistributor(ShoppingTripsdata).then(function (response) {
             $scope.ShoppingTripsTotalbyRT = response.data.total;

                $scope.ShoppingTripsRTindex = parseFloat(response.data.total);


                if (ShoppingTripsCPindex == 0 ||ShoppingTripsCPindex == null || isNaN(ShoppingTripsCPindex)) {
                  $scope.STIndex = 0;
                  $scope.cpgshoppingtripArr.push({UPC:itemnumber, STIndex:$scope.STIndex});

                }
                else {
                  $scope.STIndex = $scope.ShoppingTripsRTindex /ShoppingTripsCPindex;
                  $scope.STIndex = $scope.STIndex.toFixed(2);
                   $scope.cpgshoppingtripArr.push({UPC:itemnumber, STIndex:$scope.STIndex});
                }

          for(var i=0; i<$scope.itemsList.length;i++){
            for(var j=0; j<$scope.cpgshoppingtripArr.length; j++){
                if($scope.itemsList[i].itemNumber== $scope.cpgshoppingtripArr[j].UPC){
                   $scope.itemsList[i].linechart.shoppingtrips=$scope.cpgshoppingtripArr[j].STIndex;
                   $scope.itemsList[i].barchart.shoppingtrips=$scope.cpgshoppingtripArr[j].STIndex;
           }
          }
         }

            }, function (response) {
              console.log(response);
            }
            );
          }


    $scope.ShareOfBasketByDMA = function (upc) {
      
             var ShoppingTripsdata = {
            "aggTimeUnit": "1d",
            "startTime": $scope.ComparestartDate,
            "endTime": $scope.Compareenddate,
            // "rid" : "000",
            "filters": {
                "terms":{
                "storeId" : $scope.dmaStoreList,
                 "item.upc"  : [upc]
              }
            }
          }
          productService.getShoppingTripsforDistributor(ShoppingTripsdata).then(function (response) {
          $scope.ShoppingTripsCPindex = parseFloat(response.data.total);
          $scope.ShareOfBasketByRTDMA(upc,$scope.ShoppingTripsCPindex)
          }, function (response) {
            console.log(response);
          }
          );
        }




 /************** end of dma api calls **********/



    }]);
